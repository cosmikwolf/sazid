...
use macros::key;
pub use minimal::minimal_keymap;
...
impl<'de> Deserialize<'de> for KeyTrieNode {
    ...
    D: serde::Deserializer<'de>,
  {
    ...
    Ok(Self { map, order, ..Default::default() })
  }
}
  ...
  fn reverse_map() {
    ...
    assert_eq!(
      ...
      HashMap::from([
        ...
        ("goto_file_start".to_string(), vec![vec![key!('g'), key!('g')]]),
        ("goto_file_end".to_string(), vec![vec![key!('g'), key!('e')]]),
        ("move_line_down".to_string(), vec![vec![key!('j')], vec![key!('k')]]),
      ]),
      "Mismatch"
    )
  }
