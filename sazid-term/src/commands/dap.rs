...
impl ui::menu::Item for Thread {
  ...
  fn format(&self, thread_states: &Self::Data) -> Row {
    format!(
      "{} ({})",
      self.name,
      thread_states.get(&self.id).map(|state| state.as_str()).unwrap_or("unknown")
    )
    .into()
  }
}
    ...
    for (k, t) in &template.args {
      ...
      for (i, x) in params.iter().enumerate() {
        ...
        value = match value {
          ...
          DebugArgumentValue::String(v) => DebugArgumentValue::String(v.replace(&pattern, &param)),
          DebugArgumentValue::Array(arr) => {
            DebugArgumentValue::Array(arr.iter().map(|v| v.replace(&pattern, &param)).collect())
          },
...
pub fn dap_launch(cx: &mut Context) {
  ...
  let config = match doc.language_config().and_then(|config| config.debugger.as_ref()) {
    Some(c) => c,
    None => {
      cx.editor.set_error("No debug adapter available for language");
      return;
    },
  };
  ...
  cx.push_layer(Box::new(overlaid(Picker::new(templates, (), |cx, template, _action| {
...
pub fn dap_restart(cx: &mut Context) {
  ...
  if !debugger.capabilities().supports_restart_request.unwrap_or(false) {
    cx.editor.set_error("Debugger does not support session restarts");
    return;
  }
  if debugger.starting_request_args().is_none() {
    cx.editor.set_error("No arguments found with which to restart the sessions");
    return;
  }

  dap_callback(cx.jobs, debugger.restart(), |editor, _compositor, _resp: ()| {
...
fn debug_parameter_prompt(
  ...
  let completer = match field_type {
    "filename" => {
      |editor: &Editor, input: &str| ui::completers::filename_with_git_ignore(editor, input, false)
    },
    "directory" => {
      |editor: &Editor, input: &str| ui::completers::directory_with_git_ignore(editor, input, false)
    },
...
pub fn dap_toggle_breakpoint(cx: &mut Context) {
  ...
  let path = match doc.path() {
    ...
    None => {
      cx.editor.set_error("Can't set breakpoint: document has no path");
      return;
    },
  };
  let text = doc.text().slice(..);
...
pub fn dap_toggle_breakpoint_impl(cx: &mut Context, path: PathBuf, line: usize) {
  ...
  if let Some(pos) = breakpoints.iter().position(|breakpoint| breakpoint.line == line) {
    breakpoints.remove(pos);
  } else {
    breakpoints.push(Breakpoint { line, ..Default::default() });
  }
...
pub fn dap_continue(cx: &mut Context) {
  ...
  if let Some(thread_id) = debugger.thread_id {
    ...
    cx.editor.set_error("Currently active thread is not stopped. Switch the thread.");
  }
}
...
pub fn dap_variables(cx: &mut Context) {
  ...
  if debugger.thread_id.is_none() {
    cx.editor.set_status("Cannot access variables while target is running.");
    return;
  }
  let (frame, thread_id) = match (debugger.active_frame, debugger.thread_id) {
    (Some(frame), Some(thread_id)) => (frame, thread_id),
    _ => {
      cx.editor.set_status("Cannot find current stack frame to access variables.");
      return;
    },
  };
  ...
  let thread_frame = match debugger.stack_frames.get(&thread_id) {
    ...
    None => {
      cx.editor.set_error("Failed to get stack frame for thread: {thread_id}");
      return;
    },
  };
  let stack_frame = match thread_frame.get(frame) {
    ...
    None => {
      cx.editor.set_error("Failed to get stack frame for thread {thread_id} and frame {frame}.");
      return;
    },
  };
  ...
  for scope in scopes.iter() {
    ...
    variables.push(Spans::from(Span::styled(format!("▸ {}", scope.name), scope_style)));
  ...
  let margin = if cx.editor.popup_border() { Margin::all(1) } else { Margin::none() };
...
pub fn dap_switch_stack_frame(cx: &mut Context) {
  ...
  let picker = Picker::new(frames, (), move |cx, frame, _action| {
    ...
    let pos = debugger.stack_frames[&thread_id].iter().position(|f| f.id == frame.id);
  ...
  .with_preview(move |_editor, frame| {
    frame.source.as_ref().and_then(|source| source.path.clone()).map(|path| {
      (
        path.into(),
        Some((
          frame.line.saturating_sub(1),
