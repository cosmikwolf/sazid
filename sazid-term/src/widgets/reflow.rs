...
impl<'a, 'b> WordWrapper<'a, 'b> {
  pub fn new(
    ...
    WordWrapper { symbols, max_line_width, current_line: vec![], next_line: vec![], trim }
  }
}
...
impl<'a, 'b> LineComposer<'a> for WordWrapper<'a, 'b> {
  fn next_line(&mut self) -> Option<(&[StyledGrapheme<'a>], u16)> {
    ...
    let mut current_line_width =
      self.current_line.iter().map(|StyledGrapheme { symbol, .. }| symbol.width() as u16).sum();
      ...
      if current_line_width > self.max_line_width {
        ...
        // Push the remainder to the next line but strip leading whitespace:
        {
          ...
          if let Some(remainder_nonwhite) = remainder
            .iter()
            .position(|StyledGrapheme { symbol, .. }| !symbol.chars().all(&char::is_whitespace))
          {
            self.next_line.extend_from_slice(&remainder[remainder_nonwhite..]);
          }
        }
...
mod test {
  ...
  fn run_composer(which: Composer, text: &str, text_area_width: u16) -> (Vec<String>, Vec<u16>) {
    ...
    while let Some((styled, width)) = composer.next_line() {
      let line = styled.iter().map(|StyledGrapheme { symbol, .. }| *symbol).collect::<String>();
