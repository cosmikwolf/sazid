...
impl<'a> RenderContext<'a> {
  pub fn new(
    ...
    RenderContext { editor, doc, view, focused, spinners, parts: RenderBuffer::default() }
  }
}
...
pub fn render(context: &mut RenderContext, viewport: Rect, surface: &mut Surface) {
  ...
  surface.set_spans(viewport.x, viewport.y, &context.parts.left, context.parts.left.width() as u16);
  ...
  surface.set_spans(
    viewport.x + viewport.width.saturating_sub(context.parts.right.width() as u16),
    viewport.y,
    &context.parts.right,
...
fn append(buffer: &mut Spans, text: String, base_style: &Style, style: Option<Style>) {
  buffer.0.push(Span::styled(text, style.map_or(*base_style, |s| (*base_style).patch(s))));
}
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let (warnings, errors) = context.doc.diagnostics().iter().fold((0, 0), |mut counts, diag| {
  ...
  if warnings > 0 {
    write(context, "●".to_string(), Some(context.editor.theme.get("warning")));
    write(context, format!(" {} ", warnings), None);
  }

  if errors > 0 {
    write(context, "●".to_string(), Some(context.editor.theme.get("error")));
...
{
  let (warnings, errors) =
    context.editor.diagnostics.values().flatten().fold((0, 0), |mut counts, (diag, _)| {
      match diag.severity {
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  write(context, format!(" {} sel{} ", count, if count == 1 { "" } else { "s" }), None);
}
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  write(context, format!(" {} char{} ", tot_sel, if tot_sel == 1 { "" } else { "s" }), None);
}

fn get_position(context: &RenderContext) -> Position {
  coords_at_pos(
    context.doc.text().slice(..),
    context.doc.selection(context.view.id).primary().cursor(context.doc.text().slice(..)),
  )
}
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let title = {
    ...
    let path =
      rel_path.as_ref().map(|p| p.to_string_lossy()).unwrap_or_else(|| SCRATCH_BUFFER_NAME.into());
    format!(" {} ", path)
  };
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let title = (if context.doc.is_modified() { "[+]" } else { "   " }).to_string();
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let title = if context.doc.readonly { " [readonly] " } else { "" }.to_string();
  write(context, title, None);
}
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  write(context, sep.to_string(), Some(context.editor.theme.get("ui.statusline.separator")));
}
