...
impl Completion {
  ...
  pub fn new(
    ...
    let menu = Menu::new(items, (), move |editor: &mut Editor, item, event| {
      fn item_to_transaction(
        ...
        let (edit_offset, new_text) = if let Some(edit) = &item.text_edit {
          let edit = match edit {
            ...
            lsp::CompletionTextEdit::InsertAndReplace(item) => {
              let range = if replace_mode { item.replace } else { item.insert };
          ...
          || matches!(item.insert_text_format, Some(lsp::InsertTextFormat::SNIPPET))
        {
          match snippet::parse(&new_text) {
            ...
            Err(err) => {
              log::error!("Failed to parse snippet: {:?}, remaining output: {}", &new_text, err);
      ...
      match event {
        ...
        PromptEvent::Update if preview_completion_insert => {
          ...
          if matches!(editor.last_completion, Some(CompleteAction::Triggered)) {
            editor.last_completion =
              Some(CompleteAction::Selected { savepoint: doc.savepoint(view) })
          }
        ...
        PromptEvent::Validate => {
          ...
          let language_server = editor.language_servers.get_by_id(item.language_server_id).unwrap();
      ...
      if event != PromptEvent::Update {
        editor.handlers.trigger_signature_help(SignatureHelpInvoked::Automatic, editor);
      }
    });
    ...
    let popup =
      Popup::new(Self::ID, menu).with_scrollbar(false).ignore_escape_key(true).margin(margin);

    let (view, doc) = current_ref!(editor);
    let text = doc.text().slice(..);
    let cursor = doc.selection(view.id).primary().cursor(text);
    let offset = text.chars_at(cursor).reversed().take_while(|ch| chars::char_is_word(*ch)).count();
...
impl Component for Completion {
  ...
  fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
    ...
    let coords = view.screen_coords_at_pos(doc, text, cursor_pos).expect("cursor must be in view");
    ...
    let doc_area = if doc_width_available > 30 {
      ...
      if let Some((rel_width, rel_height)) = markdown_doc.required_size((doc_width, doc_height)) {
      ...
      let avail_height_below =
        area.height.saturating_sub(cursor_pos.max(popup_area.bottom()) + 1 /* padding */);
      ...
      Some(Ok(resolved_item)) => {
        job::dispatch(move |_, compositor| {
          if let Some(completion) =
            &mut compositor.find::<crate::ui::EditorView>().unwrap().completion
          {
