...
impl SignatureHelp {
  ...
  pub fn new(
    ...
    Self { signature, signature_doc: None, active_param_range: None, language, config_loader }
  }
...
impl Component for SignatureHelp {
  fn render(&mut self, area: Rect, surface: &mut Buffer, cx: &mut Context) {
    ...
    let active_param_span = self.active_param_range.map(|(start, end)| {
      vec![(cx.editor.theme.find_scope_index_exact("ui.selection").unwrap(), start..end)]
    });
    ...
    let sig_doc_area =
      area.clip_top(sig_text_area.height + 2).clip_bottom(u16::from(cx.editor.popup_border()));
  ...
  fn required_size(&mut self, viewport: (u16, u16)) -> Option<(u16, u16)> {
    ...
    let (width, height) = match self.signature_doc {
      Some(ref doc) => {
        ...
        let (doc_width, doc_height) = crate::ui::text::required_size(&doc_text, max_text_width);
        (sig_width.max(doc_width), sig_height + SEPARATOR_HEIGHT + doc_height)
      },
