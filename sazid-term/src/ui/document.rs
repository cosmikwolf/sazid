  ...
  for (char_idx, callback) in &mut *translated_positions {
    if *char_idx < char_pos && *char_idx >= first_visible_char_idx {
      ...
      *char_idx = usize::MAX;
      ...
      {
        callback(renderer, Position { row: pos.row, col: pos.col - renderer.col_offset })
      }
    }
  }
}
...
pub fn render_text<'t>(
  ...
  loop {
    ...
    if char_pos >= syntax_style_span.1 {
      syntax_style_span = syntax_styles.next().unwrap_or((Style::default(), usize::MAX));
    }
    if char_pos >= overlay_style_span.1 {
      overlay_style_span = overlay_styles.next().unwrap_or((Style::default(), usize::MAX));
    }
    char_pos += grapheme.doc_chars();
...
impl<'a> TextRenderer<'a> {
  pub fn new(
    ...
    let WhitespaceConfig { render: ws_render, characters: ws_chars } = &editor_config.whitespace;
  ...
  pub fn draw_indent_guides(&mut self, indent_level: usize, row: u16) {
    ...
    for i in self.starting_indent..end_indent {
      let x =
        (self.viewport.x as usize + (i * self.indent_width as usize) - self.col_offset) as u16;
      let y = self.viewport.y + row;
      debug_assert!(self.surface.in_bounds(x, y));
      self.surface.set_string(x, y, &self.indent_guide_char, self.indent_guide_style);
    }
  }
}
