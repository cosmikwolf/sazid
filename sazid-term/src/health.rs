...
pub fn general() -> std::io::Result<()> {
  ...
  writeln!(
    stdout,
    "Runtime directories: {}",
    rt_dirs.iter().map(|d| d.to_string_lossy()).collect::<Vec<_>>().join(";")
  )?;
  for rt_dir in rt_dirs.iter() {
    if let Ok(path) = std::fs::read_link(rt_dir) {
      let msg =
        format!("Runtime directory {} is symlinked to: {}", rt_dir.display(), path.display());
...
pub fn clipboard() -> std::io::Result<()> {
  ...
  match board.name().as_ref() {
    "none" => {
      writeln!(stdout, "{}", "System clipboard provider: Not installed".red())?;
      writeln!(stdout, "    {}", "For troubleshooting system clipboard issues, refer".red())?;
      writeln!(stdout, "    {}",
...
pub fn languages_all() -> std::io::Result<()> {
  ...
  let column = |item: &str, color: Color| {
    let mut data = format!(
      "{:width$}",
      item.get(..column_width - 2).map(|s| format!("{}…", s)).unwrap_or_else(|| item.to_string()),
      width = column_width,
    );
    if is_terminal {
  ...
  syn_loader_conf.language.sort_unstable_by_key(|l| l.language_id.clone());
  ...
  for lang in &syn_loader_conf.language {
    ...
    let mut cmds = lang.language_servers.iter().filter_map(|ls| {
      syn_loader_conf.language_server.get(&ls.name).map(|config| config.command.as_str())
    });
    ...
    let formatter = lang.formatter.as_ref().map(|formatter| formatter.command.as_str());
...
pub fn language(lang_str: String) -> std::io::Result<()> {
  ...
  let lang = match syn_loader_conf.language.iter().find(|l| l.language_id == lang_str) {
    Some(l) => l,
    None => {
  ...
  probe_protocol("debug adapter", lang.debugger.as_ref().map(|dap| dap.command.to_string()))?;

  probe_protocol(
    "formatter",
    lang.formatter.as_ref().map(|formatter| formatter.command.to_string()),
  )?;
