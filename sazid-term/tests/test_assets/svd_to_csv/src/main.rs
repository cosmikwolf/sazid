...
fn write_peripheral_to_csv(writer: &mut csv::Writer<File>, device: Device) -> csv::Result<()> {
  ...
  for peripheral in device.peripherals {
    ...
    println!("{}\t{}\t{}", &peripheral.name, &description, &peripheral.base_address.to_string(),);
    writer.write_record([&peripheral.name, &description, &peripheral.base_address.to_string()])?;
  }
