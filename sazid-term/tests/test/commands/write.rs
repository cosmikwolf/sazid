...
async fn test_write_quit_fail() -> anyhow::Result<()> {
  ...
  let mut app = helpers::AppBuilder::new().with_file(file.path(), None).build()?;
...
async fn test_write() -> anyhow::Result<()> {
  ...
  test_key_sequence(&mut app, Some("ithe gostak distims the doshes<ret><esc>:w<ret>"), None, false)
    .await?;
  ...
  assert_eq!(helpers::platform_line("the gostak distims the doshes"), file_content);
...
async fn test_overwrite_protection() -> anyhow::Result<()> {
  ...
  file.as_file_mut().write_all(helpers::platform_line("extremely important content").as_bytes())?;
  ...
  assert_eq!(helpers::platform_line("extremely important content"), file_content);
...
async fn test_write_quit() -> anyhow::Result<()> {
  ...
  test_key_sequence(&mut app, Some("ithe gostak distims the doshes<ret><esc>:wq<ret>"), None, true)
    .await?;
...
async fn test_write_new_path() -> anyhow::Result<()> {
  ...
  let mut app = helpers::AppBuilder::new().with_file(file1.path(), None).build()?;
  ...
  test_key_sequences(
    ...
    vec![
      (
        ...
        Some(&|app| {
          let doc = doc!(app.editor);
          assert_ne!(Some(&Severity::Error), app.editor.get_status().map(|status| status.1));
          assert_eq!(None, doc.path());
        }),
      ),
      (
        ...
        Some(&|app| {
          let doc = doc!(app.editor);
          assert_eq!(Some(&Severity::Error), app.editor.get_status().map(|status| status.1));
          assert_eq!(None, doc.path());
        }),
      ),
    ],
...
async fn test_write_insert_final_newline_unchanged_if_missing_and_false() -> anyhow::Result<()> {
  ...
  let mut app = helpers::AppBuilder::new()
    .with_config(Config {
      editor: helix_view::editor::Config { insert_final_newline: false, ..Default::default() },
      ..Default::default()
    })
    .with_file(file.path(), None)
