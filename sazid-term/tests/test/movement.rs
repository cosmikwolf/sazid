...
async fn surround_by_character() -> anyhow::Result<()> {
  ...
  test(("'so \"many 'go#[o|]#d' text\" here'", "mi\"", "'so \"#[many 'good' text|]#\" here'"))
    .await?;
...
async fn surround_inside_pair() -> anyhow::Result<()> {
  ...
  test(("((so)((many) go#[o|]#d (text))(here))", "mim", "((so)(#[(many) good (text)|]#)(here))"))
    .await?;
  test(("((so)((many) go#[o|]#d (text))(here))", "2mim", "(#[(so)((many) good (text))(here)|]#)"))
    .await?;
  ...
  test(("(so [many {go#[|od]#} text] here)", "2mim", "(so [#[|many {good} text]#] here)")).await?;
  test(("(so [many {go#[|od]#} text] here)", "3mim", "(#[|so [many {good} text] here]#)")).await?;
...
async fn surround_around_pair() -> anyhow::Result<()> {
  ...
  // Matching pairs outside selection don't match
  test(("((so)((many) go#[o|]#d (text))(here))", "mam", "((so)#[((many) good (text))|]#(here))"))
    .await?;
  test(("((so)((many) go#[o|]#d (text))(here))", "2mam", "#[((so)((many) good (text))(here))|]#"))
    .await?;
  ...
  test(("(so [many {go#[|od]#} text] here)", "2mam", "(so #[|[many {good} text]]# here)")).await?;
  test(("(so [many {go#[|od]#} text] here)", "3mam", "#[|(so [many {good} text] here)]#")).await?;
...
async fn cursor_position_append_eof() -> anyhow::Result<()> {
  ...
  test(("#[foo|]#", "abar<esc>", helpers::platform_line("#[foobar|]#\n"))).await?;
