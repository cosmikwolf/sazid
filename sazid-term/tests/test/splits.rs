    ...
    vec![
      (
        ...
        Some(&|app| {
          ...
          let doc1 =
            docs.iter().find(|doc| doc.path().unwrap() == &path::normalize(file1.path())).unwrap();

          assert_eq!("hello1", doc1.text().to_string());

          let doc2 =
            docs.iter().find(|doc| doc.path().unwrap() == &path::normalize(file2.path())).unwrap();

          assert_eq!("hello2", doc2.text().to_string());

          let doc3 =
            docs.iter().find(|doc| doc.path().unwrap() == &path::normalize(file3.path())).unwrap();
...
async fn test_changes_in_splits_apply_to_all_views() -> anyhow::Result<()> {
  ...
  test(("#[|]#", "[<space>[<space>[<space><C-w>vuuu<C-w>wUUU<C-w>quuu", "#[|]#")).await?;
  ...
  test(("#[|]#", "3[<space><C-w>v<C-s><C-w>wuu3[<space><C-w>q%d", "#[|]#")).await?;
