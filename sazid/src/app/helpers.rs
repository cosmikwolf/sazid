...
fn concatenate_option_role(role1: Option<Role>, role2: Option<Role>) -> Option<Role> {
  match (role1, role2) {
...
fn concatenate_chat_choice_streams(
  ...
  if sr1.index != sr2.index {
    ...
    Ok(ChatChoiceStream {
      ...
      finish_reason: concatenate_finish_reason(sr1.finish_reason, sr2.finish_reason)?,
      logprobs: concatenate_log_probs(sr1.logprobs.clone(), sr2.logprobs.clone()),
    })
  }
}
...
pub fn append_tool_call_chunk_to_tool_call(
  ...
  let (name, arguments) = match &chunk.function {
    Some(fc) => {
      (fc.name.clone().unwrap_or("".to_string()), fc.arguments.clone().unwrap_or("".to_string()))
    },
...
pub fn fold_stream_responses_into_assistant_message(
  ...
  let concatenated_message = smvec
    .iter()
    .skip(1)
    .try_fold(smvec[0].clone(), |acc, sr| concatenate_chat_choice_streams(&acc, sr))?;
