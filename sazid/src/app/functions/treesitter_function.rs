...
impl ToolCallTrait for TreesitterFunction {
  ...
  fn call(
    ...
    let paths =
      validate_and_extract_paths_from_argument(&function_args, session_config, true, Some(PathBuf::from("./")))?
        .unwrap();
    let query = validate_and_extract_string_argument(&function_args, "query", true)?.unwrap();
...
pub fn treesitter_query(paths: Vec<PathBuf>, query: &str) -> Result<Option<String>, ToolCallError> {
  ...
  for path in paths {
    let source_code = std::fs::read_to_string(&path)
      .map_err(|e| ToolCallError::new(format!("Error: {}  Path: {}", &e, &path.to_str().unwrap()).as_str()))?;
    let parsed = parser.parse(&source_code, None).expect("Error parsing source code");
    let root_node = parsed.root_node();
    let mut query_cursor = tree_sitter::QueryCursor::new();

    let query_matches = query_cursor.matches(query, root_node, source_code.as_bytes());
