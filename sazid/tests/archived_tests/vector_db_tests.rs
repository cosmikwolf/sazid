...
mod vector_db_tests {
  ...
  async fn setup_test_db() -> Result<VectorDB, SazidError> {
    ...
    let vectordb =
      VectorDB { client, config: VectorDBConfig { optimize_threads: 4 } };
  ...
  async fn cleanup_test_db(
    ...
    client
      .execute(
        format!("DROP TABLE IF EXISTS {} CASCADE;", table_name).as_str(),
        &[],
      )
      .await?;
    Ok(())
  ...
  #[tokio::test]
  async fn test_list_embeddings_categories(
  ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let embedding1 = FileEmbedding::new(
      vec![1.0_f64, 2.0_f64, 3.0_f64],
      "test".to_string(),
      category_names[0].into(),
    );
    let embedding2 = FileEmbedding::new(
      vec![2.0_f64, 2.0_f64, 3.0_f64],
      "test2".to_string(),
      category_names[1].into(),
    );
  ...
  async fn test_search_similar_texts() -> Result<(), Box<dyn std::error::Error>>
  {
    ...
    for i in 1..=5 {
      ...
      let embedding = FileEmbedding::new(
        vec![1.0_f64, 2.0_f64, 3.0_f64 + i as f64 / 1000000_f64],
        text,
        category_name.into(),
      );
    ...
    let similar_text_ids =
      db.search_similar_texts(category_name, &query_embedding, 5).await?;
    cleanup_test_db(&db.client, category_name).await?;
  ...
  #[tokio::test]
  async fn test_insert_text_and_retrieve_by_id(
  ) -> Result<(), Box<dyn std::error::Error>> {
    let db = setup_test_db().await?;
    ...
    let vector =
      FileEmbedding::new(embedding, text.clone(), category_name.to_string());
    ...
    let query = format!(
      ...
      VectorDB::get_table_name(category_name)
    );
    println!("query: {:?}", query);
    let text_id: i64 = db.client.query_one(&query, &[]).await?.get("id");
    let retrieved_text =
      db.get_text_by_id(category_name, text_id as i64).await?;

    assert_eq!(
      text, retrieved_text,
      "Retrieved text does not match inserted text"
    );
  ...
  async fn test_query_knn() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
      ...
      let embedding =
        FileEmbedding::new(vector, text.clone(), category_name.to_string());
      db.insert_embedding(category_name, embedding).await?;
    }

    // Convert &[f64] to a string representation PostgreSQL can understand
    let vector_as_string = [1.0, 2.0, 3.0]
      .iter()
