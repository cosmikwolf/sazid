...
mod vector_custom_type_tests {
  ...
  #[tokio::test]
  async fn test_vector_custom_type_insert_and_retrieve(
    ...
    client
      .batch_execute(
        "CREATE TABLE test_vectors (id SERIAL PRIMARY KEY, vec _float8);",
      )
      .await?;
    client
      .execute(
        "INSERT INTO test_vectors (vec) VALUES ($1)",
        &[&vector as &(dyn ToSql + Sync)],
      )
      .await?;

    // Retrieve the vector
    let row =
      client.query_one("SELECT vec FROM test_vectors LIMIT 1", &[]).await?;
