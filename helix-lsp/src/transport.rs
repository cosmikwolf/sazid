...
impl Transport {
  pub fn start(
    ...
    id: usize,
    name: String,
  ) -> (UnboundedReceiver<(usize, jsonrpc::Call)>, UnboundedSender<Payload>, Arc<Notify>) {
    let (client_tx, rx) = unbounded_channel();
    let (tx, client_rx) = unbounded_channel();
    let notify = Arc::new(Notify::new());

    let transport = Self { id, name, pending_requests: Mutex::new(HashMap::default()) };

    let transport = Arc::new(transport);

    tokio::spawn(Self::recv(transport.clone(), server_stdout, client_tx.clone()));
  ...
  async fn send_payload_to_server(&self, server_stdin: &mut BufWriter<ChildStdin>, payload: Payload) -> Result<()> {
    ...
    self.send_string_to_server(server_stdin, json, &self.name).await
  }
  ...
  async fn process_server_message(
    ...
    match msg {
      ServerMessage::Output(output) => self.process_request_response(output, language_server_name).await?,
      ServerMessage::Call(call) => {
        client_tx.send((self.id, call)).context("failed to send a message to server")?;
  ...
  async fn process_request_response(&self, output: jsonrpc::Output, language_server_name: &str) -> Result<()> {
    ...
    if let Some(tx) = self.pending_requests.lock().await.remove(&id) {
      match tx.send(result).await {
        Ok(_) => (),
        Err(_) => {
          error!("Tried sending response into a closed channel (id={:?}), original request likely timed out", id)
        },
      };
    } else {
  ...
  async fn recv(
    ...
    loop {
      match Self::recv_server_message(&mut server_stdout, &mut recv_buffer, &transport.name).await {
        Ok(msg) => {
          match transport.process_server_message(&client_tx, msg, &transport.name).await {
            Ok(_) => {},
            Err(err) => {
        ...
        Err(err) => {
          ...
          match transport.process_server_message(&client_tx, notification, &transport.name).await {
            Ok(_) => {},
            Err(err) => {
              error!("err: <- {:?}", err);
            },
  ...
  async fn send(
    ...
    fn is_initialize(payload: &Payload) -> bool {
      ...
      match payload {
        Payload::Request { value: jsonrpc::MethodCall { method, .. }, .. } if method == Initialize::METHOD => true,
        Payload::Notification(jsonrpc::Notification { method, .. }) if method == Initialized::METHOD => true,
        _ => false,
      }
    }
