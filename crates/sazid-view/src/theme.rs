...
pub static DEFAULT_THEME_DATA: Lazy<Value> = Lazy::new(|| {
  let bytes = include_bytes!("../../../lib/helix/theme.toml");
  toml::from_str(str::from_utf8(bytes).unwrap())
    .expect("Failed to parse base default theme")
});

pub static BASE16_DEFAULT_THEME_DATA: Lazy<Value> = Lazy::new(|| {
  let bytes = include_bytes!("../../../lib/helix/base16_theme.toml");
  toml::from_str(str::from_utf8(bytes).unwrap())
...
impl Loader {
  ...
  pub fn new(dirs: &[PathBuf]) -> Self {
    Self { theme_dirs: dirs.iter().map(|p| p.join("themes")).collect() }
  }
...
impl From<Value> for Theme {
  fn from(value: Value) -> Self {
    if let Value::Table(table) = value {
      ...
      Self { styles, scopes, highlights, ..Default::default() }
    } else {
...
impl<'de> Deserialize<'de> for Theme {
    ...
    D: Deserializer<'de>,
  {
    ...
    Ok(Self { styles, scopes, highlights, ..Default::default() })
  }
}
...
impl ThemePalette {
  ...
  pub fn parse_underline_style(
    value: &Value,
  ) -> Result<UnderlineStyle, String> {
    value
      .as_str()
      .and_then(|s| s.parse().ok())
    ...
    if let Value::Table(entries) = value {
      for (name, mut value) in entries {
        match name.as_str() {
          ...
          "underline" => {
            let table =
              value.as_table_mut().ok_or("Theme: underline must be table")?;
            ...
            if let Some(value) = table.remove("style") {
              *style =
                style.underline_style(Self::parse_underline_style(&value)?);
            }

            if let Some(attr) = table.keys().next() {
              return Err(format!(
                "Theme: invalid underline attribute: {attr}"
              ));
            }
          },
          "modifiers" => {
            let modifiers =
              value.as_array().ok_or("Theme: modifiers should be an array")?;
...
mod tests {
  ...
  fn test_palette() {
    ...
    let palette = ThemePalette::new(
      hashmap! { "my_color".to_string() => Color::Rgb(255, 255, 255) },
    );
    palette.parse_style(&mut style, fg).unwrap();
