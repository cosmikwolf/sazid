...
fn main() {
  if std::env::var("HELIX_DISABLE_AUTO_GRAMMAR_BUILD").is_err() {
    // std::env::set_var("HELIX_RUNTIME", "$CARGO_MANIFEST_DIR/../lib/helix");
    ...
    // let runtime_dir =
    //   std::path::PathBuf::from(manifest_dir).join("../../lib/helix");
    // let out_dir = std::path::PathBuf::from(std::env::var("OUT_DIR").unwrap())
    //   .join("../../../runtime");
    // //print out all environment variables
    //
    // match copy_dir::copy_dir(&runtime_dir, &out_dir) {
    //   Ok(_) => (),
    //   Err(err) => {
    //     eprintln!(
    //       "Failed to copy runtime directory: {}\nsrc:{}\ndest:{}",
    //       err,
