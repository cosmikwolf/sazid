...
impl<T: Component> Popup<T> {
  ...
  /// (usually the session) handle it. This is useful for popups
  ...
  pub fn get_rel_position(
    ...
    let position = self
      .position
      .get_or_insert_with(|| session.cursor().0.unwrap_or_default());
    ...
    if viewport.width <= rel_x + width {
      rel_x =
        rel_x.saturating_sub((rel_x + width).saturating_sub(viewport.width));
    }
  ...
  pub fn area(&mut self, viewport: Rect, session: &Session) -> Rect {
    ...
    let (rel_x, rel_y) = self.get_rel_position(viewport, session);
...
impl<T: Component> Component for Popup<T> {
  ...
  fn render(
    ...
    let area = self.area(viewport, cx.session);
    ...
    let render_borders = cx.session.popup_border();

    let inner =
      if self.contents.type_name().starts_with("helix_term::ui::menu::Menu") {
        area
      } else {
    ...
    if self.has_scrollbar {
      ...
      let scroll_style = cx.session.theme.get("ui.menu.scroll");
      ...
      if !fits {
        ...
        for i in 0..win_height {
          cell = &mut surface
            [(inner.right() - 1, inner.top() + (border + i) as u16)];
