...
use helix_view::{graphics::Rect, Document};
...
impl menu::Item for CompletionItem {
  ...
  fn filter_text(&self, _data: &Self::Data) -> Cow<str> {
    self.item.filter_text.as_ref().unwrap_or(&self.item.label).as_str().into()
  }
...
impl Completion {
  ...
  pub fn new(
    ...
    let preview_completion_insert = session.config().preview_completion_insert;
    let replace_mode = session.config().completion_replace;
    // Sort completion items according to their preselect status (given by the LSP server)
    items.sort_by_key(|item| !item.item.preselect.unwrap_or(false));

    // Then create the menu
    let menu =
      Menu::new(items, (), move |session: &mut Session, item, event| {
          ...
          let (edit_offset, new_text) = if let Some(edit) = &item.text_edit {
            let edit = match edit {
              ...
              lsp::CompletionTextEdit::InsertAndReplace(item) => {
                let range =
                  if replace_mode { item.replace } else { item.insert };
        ...
        macro_rules! language_server {
                ($item:expr) => {
                        ...
                        .get_by_id($item.language_server_id)
                    {
                        Some(ls) => ls,
                        None => {
                            session.set_error("completions are outdated");
                            // TODO close the completion menu somehow,
                            // currently there is no trivial way to access the SessionView to close the completion menu
                            return;
                        }
                    }
                };
        ...
        match event {
          ...
          PromptEvent::Update if preview_completion_insert => {
            ...
            // is applied to).
            if matches!(
              session.last_completion,
              Some(CompleteAction::Triggered)
            ) {
              session.last_completion = Some(CompleteAction::Selected {
          ...
          PromptEvent::Validate => {
            ...
            session.last_completion = Some(CompleteAction::Applied {
              trigger_offset,
        ...
        if event != PromptEvent::Update {
          session
            .handlers
            .trigger_signature_help(SignatureHelpInvoked::Automatic, session);
        }
      });

    let margin =
      if session.menu_border() { Margin::vertical(1) } else { Margin::none() };
    ...
    completion.popup.contents_mut().score(&completion.filter, false);
  ...
  fn replace_item(
    &mut self,
    old_item: CompletionItem,
    new_item: CompletionItem,
  ) {
    self.popup.contents_mut().replace_option(old_item, new_item);
  }

  pub fn area(&mut self, viewport: Rect, session: &Session) -> Rect {
    self.popup.area(viewport, session)
  }
}
...
impl Component for Completion {
  ...
  fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
    ...
    let markdowned = |lang: &str, detail: Option<&str>, doc: Option<&str>| {
      ...
      Markdown::new(md, cx.session.syn_loader.clone())
    };
    ...
    let popup_area = {
      let (popup_x, popup_y) = self.popup.get_rel_position(area, cx.session);
    ...
    let doc_area = if doc_width_available > 30 {
      ...
      let avail_height_above =
        cursor_pos.min(popup_area.top()).saturating_sub(1);
      let avail_height_below = area.height.saturating_sub(
        cursor_pos.max(popup_area.bottom()) + 1, /* padding */
      );
      let (y, avail_height) = if avail_height_below >= avail_height_above {
        (area.height.saturating_sub(avail_height_below), avail_height_below)
      } else {
    ...
    // clear area
    let background = cx.session.theme.get("ui.popup");
    surface.clear_with(doc_area, background);

    if cx.session.popup_border() {
...
impl AsyncHook for ResolveHandler {
  ...
  fn handle_event(
    ...
    if self.trigger.as_ref().is_some_and(|trigger| trigger == &item) {
      timeout
    } else {
  ...
  fn finish_debounce(&mut self) {
    ...
    job::dispatch_blocking(move |session, _| {
      resolve_completion_item(session, item, rx)
    })
  }
}
...
fn resolve_completion_item(
  ...
  let Some(language_server) =
    session.language_server_by_id(item.language_server_id)
  else {
    return;
  };

  let Some(future) = language_server.resolve_completion_item(item.item.clone())
  else {
    return;
  };
      ...
      Some(Ok(resolved_item)) => {
        job::dispatch(move |_, compositor| {
          if let Some(completion) =
            &mut compositor.find::<crate::ui::SessionView>().unwrap().completion
          {
