...
pub fn render(
  ...
  let base_style = if context.focused {
    context.session.theme.get("ui.statusline")
  } else {
    context.session.theme.get("ui.statusline.inactive")
  };
  ...
  let config = context.session.config();
  ...
  surface.set_spans(
    viewport.x
      + viewport.width.saturating_sub(context.parts.right.width() as u16),
    viewport.y,
    &context.parts.right,
  ...
  let edge_width =
    context.parts.left.width().max(context.parts.right.width()) as u16;
  let center_max_width =
    viewport.width.saturating_sub(2 * edge_width + 2 * spacing);
...
fn get_render_function<F>(
  element_id: StatusLineElementID,
) -> impl Fn(&mut RenderContext, F)
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  match element_id {
    ...
    helix_view::editor::StatusLineElement::FileBaseName => {
    ...
    helix_view::editor::StatusLineElement::ReadOnlyIndicator => {
      render_read_only_indicator
    },
    helix_view::editor::StatusLineElement::FileEncoding => render_file_encoding,
    helix_view::editor::StatusLineElement::FileLineEnding => {
      render_file_line_ending
    },
    ...
    helix_view::editor::StatusLineElement::PositionPercentage => {
      render_position_percentage
    },
    helix_view::editor::StatusLineElement::TotalLineNumbers => {
      render_total_line_numbers
    },
    helix_view::editor::StatusLineElement::Separator => render_separator,
    helix_view::editor::StatusLineElement::Spacer => render_spacer,
    helix_view::editor::StatusLineElement::VersionControl => {
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  write(
    ...
    if visible && config.color_modes {
      match context.session.mode() {
        Mode::Insert => Some(context.session.theme.get("ui.statusline.insert")),
        Mode::Select => Some(context.session.theme.get("ui.statusline.select")),
        Mode::Normal => Some(context.session.theme.get("ui.statusline.normal")),
      }
    } else {
      None
    },
...
{
  let (warnings, errors) =
    context.doc.diagnostics().iter().fold((0, 0), |mut counts, diag| {
  ...
  if warnings > 0 {
    write(context, "●".to_string(), Some(context.session.theme.get("warning")));
    write(context, format!(" {} ", warnings), None);
  }

  if errors > 0 {
    write(context, "●".to_string(), Some(context.session.theme.get("error")));
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let (warnings, errors) = context.session.diagnostics.values().flatten().fold(
    (0, 0),
    |mut counts, (diag, _)| {
      match diag.severity {
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  write(context, format!(" {}:{} ", position.row + 1, position.col + 1), None);
}
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  write(context, format!("{}%", (position.row + 1) * 100 / maxrows), None);
}
  ...
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  ...
  let line_ending = match context.doc.line_ending {
    ...
    _ => helix_core::LineEnding::default().name(),
  };
...
{
  let title =
    (if context.doc.is_modified() { "[+]" } else { "   " }).to_string();
...
{
  let title =
    if context.doc.readonly { " [readonly] " } else { "" }.to_string();
  write(context, title, None);
}
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let sep = &context.session.config().statusline.separator;

  write(
    context,
    sep.to_string(),
    Some(context.session.theme.get("ui.statusline.separator")),
  );
}
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  let head = context.doc.version_control_head().unwrap_or_default().to_string();
...
where
  F: Fn(&mut RenderContext, String, Option<Style>) + Copy,
{
  if let Some(reg) = context.session.selected_register {
