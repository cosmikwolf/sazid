...
use helix_view::{
  ...
  session::{CompleteAction, CursorShapeConfig},
  graphics::{Color, CursorKind, Modifier, Rect, Style},
  input::{KeyEvent, MouseButton, MouseEvent, MouseEventKind},
  keyboard::{KeyCode, KeyModifiers},
  Document, Session, Theme, View,
};
...
impl SessionView {
  ...
  pub fn render_view(
    ...
    if let Some(frame) = session.current_stack_frame() {
    ...
    for diagnostic in Self::doc_diagnostics_highlights(doc, theme) {
      ...
      overlay_highlights =
        Box::new(syntax::merge(overlay_highlights, diagnostic));
    }
    ...
    if is_focused {
      ...
      let focused_view_elements =
        Self::highlight_focused_view_elements(view, doc, theme);
    ...
    if is_focused {
      ...
      session.cursor_cache.set(Some(None));
      let update_cursor_cache =
        |_: &mut TextRenderer, pos| session.cursor_cache.set(Some(Some(pos)));
    ...
    Self::render_rulers(session, doc, view, inner, surface, theme);
    ...
    let statusline_area =
      view.area.clip_top(view.area.height.saturating_sub(1)).clip_bottom(1); // -1 from bottom to remove commandline
  ...
  pub fn render_rulers(
    ...
    let session_rulers = &session.config().rulers;
    ...
    for (i, range) in selection.iter().enumerate() {
      ...
      if range.head == range.anchor && range.head == text.len_chars() {
        if !selection_is_primary || (cursor_is_block && is_terminal_focused) {
          ...
          // BUG: If the session area loses focus while having a bar or
  ...
  pub fn render_bufferline(
    ...
    surface.clear_with(
      ...
      session
        ...
        .unwrap_or_else(|| session.theme.get("ui.statusline")),
    );

    let bufferline_active = session
      .theme
      .try_get("ui.bufferline.active")
      .unwrap_or_else(|| session.theme.get("ui.statusline.active"));

    let bufferline_inactive = session
      .theme
      .try_get("ui.bufferline")
      .unwrap_or_else(|| session.theme.get("ui.statusline.inactive"));

    let mut x = viewport.x;
    let current_doc = view!(session).doc;
    ...
    for doc in session.documents() {
      ...
      x = surface.set_stringn(x, viewport.y, text, rem_width as usize, style).0;
  ...
  pub fn render_gutter<'d>(
    ...
    for gutter_type in view.gutters() {
      let mut gutter = gutter_type.style(session, doc, view, theme, is_focused);
      let width = gutter_type.width(view, doc);
      // avoid lots of small allocations by reusing a text buffer for each line
      let mut text = String::with_capacity(width);
      let cursors = cursors.clone();
      let gutter_decoration =
        move |renderer: &mut TextRenderer, pos: LinePos| {
            ...
            gutter(pos.doc_line, selected, pos.first_visual_line, &mut text)
          {
            ...
            renderer.surface.set_style(
              Rect { x, y, width: width as u16, height: 1 },
              gutter_style,
            );
          }
          text.clear();
  ...
  pub fn cursorline_decorator(
    ...
    let line_decoration = move |renderer: &mut TextRenderer, pos: LinePos| {
      let area =
        Rect::new(viewport.x, viewport.y + pos.visual_line, viewport.width, 1);
  ...
  fn handle_keymap_event(
    ...
    cxt.session.autoinfo = self.keymaps.sticky().map(|node| node.infobox());

    let mut execute_command = |command: &commands::MappableCommand| {
      command.execute(cxt);
      helix_event::dispatch(PostCommand { command, cx: cxt });

      let current_mode = cxt.session.mode();
    ...
    match &key_result {
      ...
      KeymapResult::Pending(node) => cxt.session.autoinfo = Some(node.infobox()),
...
impl SessionView {
  fn handle_mouse_event(
    ...
    let config = cxt.session.config();
    let MouseEvent { kind, row, column, modifiers, .. } = *event;

    let pos_and_view = |session: &Session, row, column, ignore_virtual_text| {
      session.tree.views().find_map(|(view, _focus)| {
        view
          .pos_at_screen_coords(
            &session.documents[&view.doc],
            row,
            column,
            ignore_virtual_text,
          )
    ...
    let gutter_coords_and_view = |session: &Session, row, column| {
      session.tree.views().find_map(|(view, _focus)| {
        view
          .gutter_coords_at_screen_coords(row, column)
    ...
    match kind {
      MouseEventKind::Down(MouseButton::Left) => {
        let session = &mut cxt.session;

        if let Some((pos, view_id)) = pos_and_view(session, row, column, true) {
          let prev_view_id = view!(session).id;
          let doc = doc_mut!(session, &view!(session, view_id).doc);

          if modifiers == KeyModifiers::ALT {
            let selection = doc.selection(view_id).clone();
            doc.set_selection(view_id, selection.push(Range::point(pos)));
          } else if session.mode == Mode::Select {
            ...
            session.mouse_down_range = Some(primary);
            doc.set_selection(
              view_id,
          ...
          if view_id != prev_view_id {
            self.clear_completion(session);
          }

          session.focus(view_id);
          session.ensure_cursor_in_view(view_id);

          return EventResult::Consumed(None);
        }

        // if let Some((coords, view_id)) = gutter_coords_and_view(session, row, column) {
        //     session.focus(view_id);
        //
        //     let (view, doc) = current!(cxt.session);
        //
      ...
      MouseEventKind::Drag(MouseButton::Left) => {
        ...
        *primary = primary.put_cursor(doc.text().slice(..), pos, true);
        ...
        let view_id = view.id;
        cxt.session.ensure_cursor_in_view(view_id);
        EventResult::Consumed(None)
      },

      MouseEventKind::ScrollUp | MouseEventKind::ScrollDown => {
        let current_view = cxt.session.tree.focus;
        ...
        match pos_and_view(cxt.session, row, column, false) {
          Some((_, view_id)) => cxt.session.tree.focus = view_id,
          None => return EventResult::Ignored(None),
        }
        ...
        cxt.session.tree.focus = current_view;
        cxt.session.ensure_cursor_in_view(current_view);
      ...
      MouseEventKind::Up(MouseButton::Left) => {
        ...
        let should_yank = match cxt.session.mouse_down_range.take() {
      ...
      MouseEventKind::Up(MouseButton::Right) => {
        if let Some((coords, view_id)) =
          gutter_coords_and_view(cxt.session, row, column)
        {
          cxt.session.focus(view_id);

          let (view, doc) = current!(cxt.session);
          if let Some(pos) = view.pos_at_visual_coords(
            doc,
            coords.row as u16,
            coords.col as u16,
            true,
...
impl Component for SessionView {
  fn handle_event(
    ...
    match event {
      Event::Paste(contents) => {
        cx.count = cx.session.count;
        commands::paste_bracketed_value(&mut cx, contents.clone());
        cx.session.count = None;

        let config = cx.session.config();
        let mode = cx.session.mode();
      ...
      Event::Key(mut key) => {
        cx.session.reset_idle_timer();
        canonicalize_key(&mut key);

        // clear status
        cx.session.status_msg = None;
            ...
            Mode::Insert => {
              ...
              if let Some(completion) = &mut self.completion {
                ...
                if let Some(callback) = res {
                  if callback.is_some() {
                    // assume close_fn
                    self.clear_completion(cx.session);
                  }
                }
              }
      ...
      Event::FocusLost => {
        if context.session.config().auto_save {
          if let Err(e) = commands::typed::write_all_impl(context, false, false)
          {
            context.session.set_error(format!("{}", e));
          }
        }
  ...
  fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
    ...
    surface.set_style(area, cx.session.theme.get("ui.background"));
    let config = cx.session.config();

    // check if bufferline should be rendered
    use helix_view::session::BufferLine;
    let use_bufferline = match config.bufferline {
      BufferLine::Always => true,
      BufferLine::Multiple if cx.session.documents.len() > 1 => true,
      _ => false,
    };

    // -1 for commandline and -1 for bufferline
    let mut session_area = area.clip_bottom(1);
    if use_bufferline {
      session_area = session_area.clip_top(1);
    }

    // if the terminal size suddenly changed, we need to trigger a resize
    cx.session.resize(session_area);

    if use_bufferline {
      Self::render_bufferline(cx.session, area.with_height(1), surface);
    }

    for (view, is_focused) in cx.session.tree.views() {
      let doc = cx.session.document(view.doc).unwrap();
      self.render_view(cx.session, doc, view, area, surface, is_focused);
    }

    if config.auto_info {
      if let Some(mut info) = cx.session.autoinfo.take() {
        info.render(area, surface, cx);
        cx.session.autoinfo = Some(info)
      }
    }
    ...
    // render status msg
    if let Some((status_msg, severity)) = &cx.session.status_msg {
      status_msg_width = status_msg.width();
      use helix_view::session::Severity;
      let style = if *severity == Severity::Error {
        cx.session.theme.get("error")
      } else {
        cx.session.theme.get("ui.text")
      };
    ...
    if area.width.saturating_sub(status_msg_width as u16) > key_width {
      ...
      if let Some(count) = cx.session.count {
      ...
      let style = cx.session.theme.get("ui.text");
      let macro_width = if cx.session.macro_recording.is_some() { 3 } else { 0 };
      surface.set_string(
      ...
      if let Some((reg, _)) = cx.session.macro_recording {
        let disp = format!("[{}]", reg);
        let style = style
