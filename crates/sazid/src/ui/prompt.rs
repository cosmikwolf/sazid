...
use helix_core::{
  unicode::segmentation::GraphemeCursor, unicode::width::UnicodeWidthStr,
  Position,
};
...
type CompletionFn = Box<dyn FnMut(&Session, &str) -> Vec<Completion>>;
...
impl Prompt {
  pub fn new(
    prompt: Cow<'static, str>,
    history_register: Option<char>,
    completion_fn: impl FnMut(&Session, &str) -> Vec<Completion> + 'static,
  ...
  pub fn with_line(mut self, line: String, session: &Session) -> Self {
    let cursor = line.len();
    self.line = line;
    self.cursor = cursor;
    self.recalculate_completion(session);
    self
  }
  ...
  pub fn recalculate_completion(&mut self, session: &Session) {
    self.exit_selection();
    self.completion = (self.completion_fn)(session, &self.line);
  }
  ...
  fn eval_movement(&self, movement: Movement) -> usize {
    match movement {
      Movement::BackwardChar(rep) => {
        ...
        for _ in 0..rep {
          let mut cursor =
            GraphemeCursor::new(position, self.line.len(), false);
      ...
      Movement::BackwardWord(rep) => {
        let char_indices: Vec<(usize, char)> =
          self.line.char_indices().collect();
  ...
  pub fn insert_char(&mut self, c: char, cx: &Context) {
    ...
    self.recalculate_completion(cx.session);
  }

  pub fn insert_str(&mut self, s: &str, session: &Session) {
  ...
  pub fn delete_char_backwards(&mut self, session: &Session) {
  ...
  pub fn delete_char_forwards(&mut self, session: &Session) {
  ...
  pub fn delete_word_backwards(&mut self, session: &Session) {
  ...
  pub fn delete_word_forwards(&mut self, session: &Session) {
  ...
  pub fn kill_to_start_of_line(&mut self, session: &Session) {
  ...
  pub fn kill_to_end_of_line(&mut self, session: &Session) {
  ...
  pub fn clear(&mut self, session: &Session) {
    self.line.clear();
    self.cursor = 0;
    self.recalculate_completion(session);
  }
  ...
  //   let mut values = match cx.session.registers.read(register, cx.session) {
  ...
  //       self.history_pos.unwrap_or_else(|| values.len()).saturating_sub(1)
  //     },
  //   }
  //   .min(end);
  //
  ...
  pub fn render_prompt(
    ...
    if self.line.is_empty() {
      ...
      if let Some(suggestion) = self
        .history_register
        .and_then(|reg| cx.session.registers.first(reg, cx.session))
      {
        surface.set_string(
          line_area.x,
          line_area.y,
...
impl Component for Prompt {
  fn handle_event(&mut self, event: &Event, cx: &mut Context) -> EventResult {
    let event = match event {
      Event::Paste(data) => {
        self.insert_str(data, cx.session);
        self.recalculate_completion(cx.session);
    ...
    match event {
      ...
      ctrl!('w') | alt!(Backspace) | ctrl!(Backspace) => {
        self.delete_word_backwards(cx.session);
        (self.callback_fn)(cx, &self.line, PromptEvent::Update);
      },
      alt!('d') | alt!(Delete) | ctrl!(Delete) => {
        self.delete_word_forwards(cx.session);
        (self.callback_fn)(cx, &self.line, PromptEvent::Update);
      },
      ctrl!('k') => {
        self.kill_to_end_of_line(cx.session);
        (self.callback_fn)(cx, &self.line, PromptEvent::Update);
      },
      ctrl!('u') => {
        self.kill_to_start_of_line(cx.session);
        (self.callback_fn)(cx, &self.line, PromptEvent::Update);
      },
      ctrl!('h') | key!(Backspace) | shift!(Backspace) => {
        self.delete_char_backwards(cx.session);
        (self.callback_fn)(cx, &self.line, PromptEvent::Update);
      },
      ctrl!('d') | key!(Delete) => {
        self.delete_char_forwards(cx.session);
        (self.callback_fn)(cx, &self.line, PromptEvent::Update);
      },
      ctrl!('s') => {
        ...
        if !line.is_empty() {
          self.insert_str(line.as_str(), cx.session);
          (self.callback_fn)(cx, &self.line, PromptEvent::Update);
        }
      },
      key!(Enter) => {
        if self.selection.is_some()
          && self.line.ends_with(std::path::MAIN_SEPARATOR)
        {
          self.recalculate_completion(cx.session);
        } else {
          let last_item = self
          ...
          let input = if self.line.is_empty() {
            ...
            if last_item != self.line {
              ...
              if let Some(register) = self.history_register {
                if let Err(err) =
                  cx.session.registers.push(register, self.line.clone())
                {
