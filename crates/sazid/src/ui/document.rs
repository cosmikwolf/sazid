...
use helix_core::doc_formatter::{
  DocumentFormatter, GraphemeSource, TextFormat,
};
...
pub type TranslatedPosition<'a> =
  (usize, Box<dyn FnMut(&mut TextRenderer, Position) + 'a>);
...
pub fn render_document(
  ...
  let mut renderer =
    TextRenderer::new(surface, doc, theme, offset.horizontal_offset, viewport);
  render_text(
    &mut renderer,
    doc.text().slice(..),
    offset,
    ...
    if *char_idx < char_pos && *char_idx >= first_visible_char_idx {
      ...
      *char_idx = usize::MAX;
        ...
        && pos.col - renderer.col_offset < renderer.viewport.width as usize
      {
        callback(
          renderer,
          Position { row: pos.row, col: pos.col - renderer.col_offset },
        )
      }
    }
  }
...
pub fn render_text<'t>(
  ...
  let mut syntax_style_span =
    syntax_styles.next().unwrap_or_else(|| (Style::default(), usize::MAX));
  let mut overlay_style_span =
    overlay_styles.next().unwrap_or_else(|| (Style::default(), usize::MAX));
  ...
  loop {
    ...
    if pos.row < row_off {
      if char_pos >= syntax_style_span.1 {
        syntax_style_span =
          if let Some(syntax_style_span) = syntax_styles.next() {
            syntax_style_span
          } else {
            break;
          }
      ...
      if char_pos >= overlay_style_span.1 {
        overlay_style_span =
          if let Some(overlay_style_span) = overlay_styles.next() {
            overlay_style_span
          } else {
            break;
          }
    ...
    if char_pos >= syntax_style_span.1 {
      syntax_style_span =
        syntax_styles.next().unwrap_or((Style::default(), usize::MAX));
    }
    if char_pos >= overlay_style_span.1 {
      overlay_style_span =
        overlay_styles.next().unwrap_or((Style::default(), usize::MAX));
    }
    char_pos += grapheme.doc_chars();
    ...
    renderer.draw_grapheme(
      grapheme.grapheme,
      GraphemeStyle { syntax_style, overlay_style },
      is_virtual,
  ...
  renderer
    .draw_indent_guides(last_line_indent_level, last_line_pos.visual_line);
  for line_decoration in &mut *line_decorations {
    line_decoration.render_foreground(renderer, last_line_pos, char_pos);
  }
}
...
impl<'a> TextRenderer<'a> {
  pub fn new(
    ...
    let session_config = doc.config.load();
    let WhitespaceConfig { render: ws_render, characters: ws_chars } =
      &session_config.whitespace;
    ...
    TextRenderer {
      surface,
      indent_guide_char: session_config.indent_guides.character.into(),
      newline,
      nbsp,
      space,
      tab,
      ...
      starting_indent: col_offset / indent_width as usize
        ...
        + session_config.indent_guides.skip_levels as usize,
      ...
      text_style,
      draw_indent_guides: session_config.indent_guides.render,
      viewport,
      col_offset,
    }
  }
  ...
  pub fn draw_grapheme(
    ...
    let tab = if is_virtual { &self.virtual_tab } else { &self.tab };
  ...
  pub fn draw_indent_guides(&mut self, indent_level: usize, row: u16) {
    ...
    let end_indent = min(
      ...
      self.col_offset
        ...
        + (self.indent_width as usize - 1),
    ) / self.indent_width as usize;

    for i in self.starting_indent..end_indent {
      let x = (self.viewport.x as usize + (i * self.indent_width as usize)
