...
type FileCallback<T> = Box<dyn Fn(&Session, &T) -> Option<FileLocation>>;
...
// from borrowing a document already opened in the session.
pub enum Preview<'picker, 'session> {
...
impl Preview<'_, '_> {
  fn document(&self) -> Option<&Document> {
    match self {
      Preview::SessionDocument(doc) => Some(doc),
  ...
  fn placeholder(&self) -> &str {
    match *self {
      Self::SessionDocument(_) => "<Invalid file location>",
...
fn item_to_nucleo<T: Item>(
  ...
  let row = item.format(session_data);
...
impl<T: Item> Injector<T> {
  pub fn push(&self, item: T) -> Result<(), InjectorShutdown> {
    ...
    if let Some((item, matcher_text)) = item_to_nucleo(item, &self.session_data)
    {
...
impl<T: Item + 'static> Picker<T> {
  pub fn stream(session_data: T::Data) -> (Nucleo<T>, Injector<T>) {
  ...
  pub fn new(
    ...
    for item in options {
      if let Some((item, matcher_text)) = item_to_nucleo(item, &session_data) {
        injector.push(item, |dst| dst[0] = matcher_text);
      }
    }
    Self::with(
      matcher,
      Arc::new(session_data),
  ...
  pub fn with_stream(
    ...
    Self::with(matcher, injector.session_data, injector.shutown, callback_fn)
  }
  ...
  fn with(
    ...
    let prompt = Prompt::new(
      ...
      None,
      ui::completers::none,
      |_session: &mut Context, _pattern: &str, _event: PromptEvent| {},
    );
  ...
  pub fn with_preview(
    mut self,
    preview_fn: impl Fn(&Session, &T) -> Option<FileLocation> + 'static,
  ) -> Self {
  ...
  pub fn set_options(&mut self, new_options: Vec<T>) {
    ...
    for item in new_options {
      if let Some((item, matcher_text)) =
        item_to_nucleo(item, &self.session_data)
      {
        injector.push(item, |dst| dst[0] = matcher_text);
      }
    }
  ...
  fn current_file(&self, session: &Session) -> Option<FileLocation> {
    self
      .selection()
      .and_then(|current| (self.file_fn.as_ref()?)(session, current))
      .map(|(path_or_id, line)| (path_or_id.get_canonicalized(), line))
  }

  /// Get (cached) preview for a given path. If a document corresponding
  /// to the path is already open in the session, it is used instead.
  fn get_preview<'picker, 'session>(
    &'picker mut self,
    path_or_id: PathOrId,
    session: &'session Session,
  ) -> Preview<'picker, 'session> {
    match path_or_id {
      PathOrId::Path(path) => {
        let path = &path;
        if let Some(doc) = session.document_by_path(path) {
          return Preview::SessionDocument(doc);
        }
        ...
        let preview = data
          .map(|(metadata, content_type)| {
            match (metadata.len(), content_type) {
              ...
              _ => Document::open(path, None, None, session.config.clone())
      ...
      PathOrId::Id(id) => {
        let doc = session.documents.get(&id).unwrap();
        Preview::SessionDocument(doc)
      },
    }
  }

  fn handle_idle_timeout(&mut self, cx: &mut Context) -> EventResult {
    let Some((current_file, _)) = self.current_file(cx.session) else {
      return EventResult::Consumed(None);
    };

    // Try to find a document in the cache
    let doc = match &current_file {
      PathOrId::Id(doc_id) => doc_mut!(cx.session, doc_id),
    ...
    if doc.language_config().is_none() {
      if let Some(language_config) =
        doc.detect_language_config(&cx.session.syn_loader.load())
      {
        doc.language = Some(language_config.clone());
        let text = doc.text().clone();
        let loader = cx.session.syn_loader.clone();
        let job = tokio::task::spawn_blocking(move || {
          ...
          let callback =
            move |session: &mut Session, compositor: &mut Compositor| {
              ...
              // Try to find a document in the cache
              let doc = match current_file {
                PathOrId::Id(doc_id) => doc_mut!(session, &doc_id),
                PathOrId::Path(path) => {
                  match picker.preview_cache.get_mut(&path) {
                    Some(CachedPreview::Document(ref mut doc)) => {
                      let diagnostics = Session::doc_diagnostics(
                        &session.language_servers,
                        &session.diagnostics,
                        doc,
                      );
          ...
          Callback::SessionCompositor(Box::new(callback))
        });
  ...
  fn render_picker(
    ...
    let text_style = cx.session.theme.get("ui.text");
    let selected = cx.session.theme.get("ui.text.focus");
    let highlight_style =
      cx.session.theme.get("special").add_modifier(Modifier::BOLD);

    // -- Render the frame:
    // clear area
    let background = cx.session.theme.get("ui.background");
    ...
    // -- Separator
    let sep_style = cx.session.theme.get("ui.background.separator");
    ...
    let options = snapshot.matched_items(offset..end).map(|item| {
      ...
      indices.dedup();
      let mut row = item.data.format(&self.session_data);
  ...
  fn render_preview(
    ...
    // clear area
    let background = cx.session.theme.get("ui.background");
    let text = cx.session.theme.get("ui.text");
    surface.clear_with(area, background);
    ...
    if let Some((path, range)) = self.current_file(cx.session) {
      let preview = self.get_preview(path, cx.session);
      ...
      let syntax_highlights = SessionView::doc_syntax_highlights(
        doc,
        offset.anchor,
        area.height,
        &cx.session.theme,
      ...
      let mut overlay_highlights =
        SessionView::empty_highlight_iter(doc, offset.anchor, area.height);
      for spans in
        SessionView::doc_diagnostics_highlights(doc, &cx.session.theme)
      {
        if spans.is_empty() {
          continue;
        }
      ...
      if let Some((start, end)) = range {
        let style = cx
          ...
          .unwrap_or_else(|| cx.session.theme.get("ui.selection"));
...
impl<T: Item + 'static + Send + Sync> Component for Picker<T> {
  fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
    ...
    let picker_width = if render_preview { area.width / 2 } else { area.width };
  ...
  fn handle_event(&mut self, event: &Event, ctx: &mut Context) -> EventResult {
    ...
    let close_fn = |picker: &mut Self| {
      ...
      let callback: compositor::Callback =
        if picker.matcher.snapshot().item_count() > 100_000 {
  ...
  fn cursor(
    ...
    self.prompt.cursor(area, session)
  }
...
pub type DynQueryCallback<T> = Box<
  dyn Fn(String, &mut Session) -> BoxFuture<'static, anyhow::Result<Vec<T>>>,
>;
...
impl<T: ui::menu::Item + Send + Sync> DynamicPicker<T> {
  pub fn new(
    ...
    Self { file_picker, query_callback, query: String::new() }
  }
}
...
impl<T: Item + Send + Sync + 'static> Component for DynamicPicker<T> {
  ...
  fn handle_event(&mut self, event: &Event, cx: &mut Context) -> EventResult {
    ...
    let new_options =
      (self.query_callback)(current_query.to_owned(), cx.session);

    cx.jobs.callback(async move {
      let new_options = new_options.await?;
      let callback =
        Callback::SessionCompositor(Box::new(move |session, compositor| {
