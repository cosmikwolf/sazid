...
pub fn prompt(
  ...
  completion_fn: impl FnMut(&Session, &str) -> Vec<prompt::Completion> + 'static,
...
pub fn raw_regex_prompt(
              ...
              if event == PromptEvent::Validate {
                let callback = async move {
                  let call: job::Callback =
                    Callback::SessionCompositor(Box::new(
  ...
  .with_language("regex", std::sync::Arc::clone(&cx.session.syn_loader));
...
pub fn file_picker(root: PathBuf, config: &SessionConfig) -> Picker<PathBuf> {
  ...
  // We want to exclude files that the session can't handle yet
  ...
  let picker =
    Picker::new(Vec::new(), root, move |cx, path: &PathBuf, action| {
      if let Err(e) = cx.session.open(path, action) {
        let err = if let Some(err) = e.source() {
          format!("{}", err)
        } else {
          format!("unable to open \"{}\"", path.display())
        };
        cx.session.set_error(err);
      }
    })
    .with_preview(|_session, path| Some((path.clone().into(), None)));
...
pub mod completers {
  ...
  pub type Completer = fn(&Session, &str) -> Vec<Completion>;

  pub fn none(_session: &Session, _input: &str) -> Vec<Completion> {
    Vec::new()
  }

  pub fn buffer(session: &Session, input: &str) -> Vec<Completion> {
    let names = session.documents.values().map(|doc| {
      doc
        .relative_path()
  ...
  pub fn theme(_session: &Session, input: &str) -> Vec<Completion> {
    let mut names =
  ...
  pub fn setting(_session: &Session, input: &str) -> Vec<Completion> {
    static KEYS: Lazy<Vec<String>> = Lazy::new(|| {
      let mut keys = Vec::new();
      let json = serde_json::json!(SessionConfig::default());
  ...
  pub fn filename(session: &Session, input: &str) -> Vec<Completion> {
    filename_with_git_ignore(session, input, true)
  }
  ...
  pub fn filename_with_git_ignore(
    ...
    filename_impl(session, input, git_ignore, |entry| {
  ...
  pub fn language(session: &Session, input: &str) -> Vec<Completion> {
    let text: String = "text".into();

    let loader = session.syn_loader.load();
  ...
  pub fn directory(session: &Session, input: &str) -> Vec<Completion> {
    directory_with_git_ignore(session, input, true)
  }
