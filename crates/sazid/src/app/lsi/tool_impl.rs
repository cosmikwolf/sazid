...
use super::{
  interface::LanguageServerInterface, query::LsiQuery,
  symbol_types::SerializableSourceSymbol,
};
...
impl LanguageServerInterface {
  pub async fn goto_type_definition(
    ...
    let symbol =
      workspace.query_symbol_by_id(&symbol_id).unwrap_or_else(|| {
        panic!("could not find symbol with id {:#?}", symbol_id)
      });
  ...
  pub fn get_diagnostics(
    ...
    let file_regex = lsi_query.file_path_regex.as_ref().map(|pattern| {
    ...
    let diagnostics = workspace
      ...
      .filter(|file| match file_regex.clone() {
        Some(file_regex) => {
          file_regex.is_match(&file.file_path.display().to_string())
        },
        None => true,
      })
      .map(|file| {
            ...
            .filter(|d| match lsi_query.diagnostic_severity {
              Some(ref s) => match d.severity {
                Some(DiagnosticSeverity::ERROR) => {
                  s.include_errors.unwrap_or(true)
                },
                Some(DiagnosticSeverity::WARNING) => {
                  s.include_warnings.unwrap_or(true)
                },
                Some(DiagnosticSeverity::INFORMATION) => {
                  s.include_information.unwrap_or(true)
                },
                Some(DiagnosticSeverity::HINT) => {
                  s.include_hints.unwrap_or(true)
                },
  ...
  pub fn get_workspace_files(
    ...
    let pattern = lsi_query.file_path_regex.as_ref().map(|pattern| {
      regex::Regex::new(pattern).expect("invalid regex pattern")
    });
    ...
    match pattern {
      Some(pattern) => {
        let files = workspace
          ...
          .iter()
          .filter(|file| {
            pattern.is_match(&file.file_path.display().to_string())
          })
  ...
  fn get_workspace(&self, lsi_query: &LsiQuery) -> anyhow::Result<&Workspace> {
    match self
      .workspaces
      .iter()
      .find(|w| w.workspace_path == lsi_query.workspace_root)
    {
      Some(workspace) => Ok(workspace),
      None => Err(anyhow::anyhow!(
        "no workspace found at {:#?}",
        lsi_query.workspace_root
      )),
    }
  }
}
