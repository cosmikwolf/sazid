...
use helix_lsp::{
  block_on,
  lsp::{
    self, CodeAction, CodeActionOrCommand, CodeActionTriggerKind,
    DiagnosticSeverity, NumberOrString,
  },
  util::{
    diagnostic_to_lsp_diagnostic, lsp_range_to_range, range_to_lsp_range,
  },
...
use super::{align_view, push_jump, Align, Context};

use helix_core::{
  syntax::LanguageServerFeature, text_annotations::InlineAnnotation, Selection,
};
use helix_stdx::path;
use helix_view::{
...
use crate::{
  ...
  job::Callback,
  ui::{
    self, overlay::overlaid, DynamicPicker, FileLocation, Picker, Popup,
    PromptEvent,
  },
};
...
/// Using this macro in a context where the session automatically queries the LSP
...
macro_rules! language_server_with_feature {
  ($session:expr, $doc:expr, $feature:expr) => {{
  ...
  fn format(&self, current_doc_path: &Self::Data) -> Row {
    if current_doc_path.as_ref() == Some(&self.symbol.location.uri) {
      ...
      match self.symbol.location.uri.to_file_path() {
        ...
        Err(_) => {
          format!("{} ({})", &self.symbol.name, &self.symbol.location.uri)
            .into()
        },
      }
    }
...
fn jump_to_location(
  ...
  jump_to_position(session, &path, location.range, offset_encoding, action);
}
...
fn jump_to_position(
  ...
  let doc = match session.open(path, action) {
    Ok(id) => doc_mut!(session, &id),
    Err(err) => {
...
fn sym_picker(
  ...
  .with_preview(move |_session, item| {
...
fn diag_picker(
  ...
  for (path, diags) in diagnostics {
    ...
    for (diag, ls) in diags {
      if let Some(ls) = cx.session.language_server_by_id(ls) {
  ...
  let styles = DiagnosticStyles {
    hint: cx.session.theme.get("hint"),
    info: cx.session.theme.get("info"),
    warning: cx.session.theme.get("warning"),
    error: cx.session.theme.get("error"),
  };

  Picker::new(
    flat_diag,
    (styles, format),
    move |cx, PickerDiagnostic { path, diag, offset_encoding }, action| {
      jump_to_position(cx.session, path, diag.range, *offset_encoding, action)
    },
  )
  .with_preview(move |_session, PickerDiagnostic { path, diag, .. }| {
    let line =
      Some((diag.range.start.line as usize, diag.range.end.line as usize));
...
pub fn symbol_picker(cx: &mut Context) {
  ...
  cx.jobs.callback(async move {
    ...
    let call = move |_session: &mut Session, compositor: &mut Compositor| {
...
pub fn workspace_symbol_picker(cx: &mut Context) {
  ...
  let get_symbols = move |pattern: String, session: &mut Session| {
    let doc = doc!(session);
    let mut seen_language_servers = HashSet::new();
    let mut futures: FuturesUnordered<_> = doc
      .language_servers_with_feature(LanguageServerFeature::WorkspaceSymbols)
      ...
      .map(|language_server| {
        let request =
          language_server.workspace_symbols(pattern.clone()).unwrap();
        let offset_encoding = language_server.offset_encoding();
        async move {
          let json = request.await?;

          let response = serde_json::from_value::<
            Option<Vec<lsp::SymbolInformation>>,
          >(json)?
          .unwrap_or_default()
          .into_iter()
          .map(|symbol| SymbolInformationItem { symbol, offset_encoding })
  ...
  let current_url = doc.url();
  let initial_symbols = get_symbols("".to_owned(), cx.session);
...
pub fn diagnostics_picker(cx: &mut Context) {
  ...
  if let Some(current_path) = doc.path() {
    let diagnostics =
      cx.session.diagnostics.get(current_path).cloned().unwrap_or_default();
...
pub fn workspace_diagnostics_picker(cx: &mut Context) {
  ...
  let diagnostics = cx.session.diagnostics.clone();
...
///
/// The order used here is modeled after the [vscode sourcecode](https://github.com/microsoft/vscode/blob/eaec601dd69aeb4abb63b9601a6f44308c8d8c6e/src/vs/session/contrib/codeAction/browser/codeActionWidget.ts>)
...
pub fn code_action(cx: &mut Context) {
  ...
  let mut futures: FuturesUnordered<_> = doc
        ...
        .map(|(request, ls_id)| async move {
            ...
            actions.sort_by(|action1, action2| {
                ...
                // https://github.com/microsoft/vscode/blob/eaec601dd69aeb4abb63b9601a6f44308c8d8c6e/src/vs/session/contrib/codeAction/browser/codeAction.ts
  ...
  if futures.is_empty() {
    cx.session.set_error("No configured language server supports code actions");
    return;
  }
  ...
  cx.jobs.callback(async move {
    ...
    let call = move |session: &mut Session, compositor: &mut Compositor| {
      if actions.is_empty() {
        session.set_error("No code actions available");
        return;
      }
      let mut picker =
        ui::Menu::new(actions, (), move |session, action, event| {
          ...
          let Some(language_server) =
            session.language_server_by_id(action.language_server_id)
          else {
            session.set_error("Language Server disappeared");
            return;
          };
          let offset_encoding = language_server.offset_encoding();
          ...
          match &action.lsp_item {
            ...
            lsp::CodeActionOrCommand::CodeAction(code_action) => {
              ...
              if let Some(ref workspace_edit) = resolved_code_action.edit {
                let _ =
                  session.apply_workspace_edit(offset_encoding, workspace_edit);
              }
      ...
      let margin = if session.menu_border() {
        Margin::vertical(1)
      } else {
        Margin::none()
      };
      ...
      let popup =
        Popup::new("code-action", picker).with_scrollbar(false).margin(margin);
...
pub fn execute_lsp_command(
  ...
  let future = match session
    .language_server_by_id(language_server_id)
    .and_then(|language_server| language_server.command(cmd))
  {
    Some(future) => future,
    None => {
      session.set_error("Language server does not support executing commands");
      return;
    },
  };
...
impl ToString for ApplyEditErrorKind {
  fn to_string(&self) -> String {
    match self {
      ...
      ApplyEditErrorKind::UnknownURISchema => {
        "URI schema not supported".to_string()
      },
...
fn goto_impl(
  ...
  match locations.as_slice() {
    [location] => {
      jump_to_location(session, location, offset_encoding, Action::Replace);
    },
    [] => unreachable!("`locations` should be non-empty for `goto_impl`"),
    _locations => {
      let picker =
        Picker::new(locations, cwdir, move |cx, location, action| {
          jump_to_location(cx.session, location, offset_encoding, action)
        })
        .with_preview(move |_session, location| {
  ...
  F: Future<Output = helix_lsp::Result<serde_json::Value>> + 'static + Send,
{
  ...
  let language_server = language_server_with_feature!(cx.session, doc, feature);
  let offset_encoding = language_server.offset_encoding();
  let pos = doc.position(view.id, offset_encoding);
  let future =
    request_provider(language_server, pos, doc.identifier()).unwrap();
  ...
  cx.callback(
    ...
    move |session,
      ...
      if items.is_empty() {
        session.set_error("No definition found.");
      } else {
        goto_impl(session, compositor, items, offset_encoding);
      }
    },
  );
}
...
pub fn goto_reference(cx: &mut Context) {
  ...
  cx.callback(
    future,
    move |session, compositor, response: Option<Vec<lsp::Location>>| {
      let items = response.unwrap_or_default();
      if items.is_empty() {
        session.set_error("No references found.");
      } else {
        goto_impl(session, compositor, items, offset_encoding);
      }
    },
...
pub fn signature_help(cx: &mut Context) {
  cx.session
    .handlers
    .trigger_signature_help(SignatureHelpInvoked::Manual, cx.session)
}
...
pub fn hover(cx: &mut Context) {
  ...
  cx.callback(
    future,
    move |session, compositor, response: Option<lsp::Hover>| {
...
pub fn rename_symbol(cx: &mut Context) {
  fn get_prefill_from_word_boundary(session: &Session) -> String {
    let (view, doc) = current_ref!(session);
  ...
  fn get_prefill_from_lsp_response(
    ...
    match response {
      ...
      Some(lsp::PrepareRenameResponse::DefaultBehavior { .. }) => {
        Ok(get_prefill_from_word_boundary(session))
      },
    ...
    let prompt = ui::Prompt::new(
      ...
      move |cx: &mut compositor::Context, input: &str, event: PromptEvent| {
        ...
        match block_on(future) {
          Ok(edits) => {
            let _ = cx.session.apply_workspace_edit(offset_encoding, &edits);
          },
          Err(err) => cx.session.set_error(err.to_string()),
        }
      },
    )
    .with_line(prefill, session);
  ...
  if let Some(language_server) = language_server_with_prepare_rename_support {
    ...
    cx.callback(
      ...
      move |session,
        ...
        let prompt = create_rename_prompt(session, prefill, Some(ls_id));

        compositor.push(prompt);
      },
    );
  } else {
    let prefill = get_prefill_from_word_boundary(cx.session);
    let prompt = create_rename_prompt(cx.session, prefill, None);
...
pub fn compute_inlay_hints_for_all_views(
  ...
  if !session.config().lsp.display_inlay_hints {
    return;
  }

  for (view, _) in session.tree.views() {
    let doc = match session.documents.get(&view.doc) {
      Some(doc) => doc,
      None => continue,
    };
...
fn compute_inlay_hints_for_view(
  ...
  std::pin::Pin<
    Box<impl Future<Output = Result<crate::job::Callback, anyhow::Error>>>,
  >,
> {
  let view_id = view.id;
  let doc_id = view.doc;
  ...
  let new_doc_inlay_hints_id = DocumentInlayHintsId { first_line, last_line };
  ...
  let callback = super::make_job_callback(
    language_server.text_document_range_inlay_hints(
      doc.identifier(),
      range,
      None,
    )?,
    move |session, _compositor, response: Option<Vec<lsp::InlayHint>>| {
      // The config was modified or the window was closed while the request was in flight
      if !session.config().lsp.display_inlay_hints
        || session.tree.try_get(view_id).is_none()
      {
        return;
      }

      // Add annotations to relevant document, not the current one (it may have changed in between)
      let doc = match session.documents.get_mut(&doc_id) {
        Some(doc) => doc,
        None => return,
      };
