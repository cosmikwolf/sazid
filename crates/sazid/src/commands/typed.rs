...
impl CommandSignature {
  const fn none() -> Self {
    Self { positional_args: &[], var_args: completers::none }
  }

  const fn positional(completers: &'static [Completer]) -> Self {
    Self { positional_args: completers, var_args: completers::none }
  }

  const fn all(completer: Completer) -> Self {
    Self { positional_args: &[], var_args: completer }
  }
}
...
fn quit(
  ...
  if cx.session.tree.views().count() == 1 {
    buffers_remaining_impl(cx.session)?
  }

  cx.block_try_flush_writes()?;
  cx.session.close(view!(cx.session).id);
  ...
  for arg in args {
    ...
    if let Ok(true) = std::fs::canonicalize(&path).map(|p| p.is_dir()) {
      let callback = async move {
        let call: job::Callback = job::Callback::SessionCompositor(Box::new(
          move |session: &mut Session, compositor: &mut Compositor| {
            let picker = ui::file_picker(path.into_owned(), &session.config());
      ...
      let _ = cx.session.open(&path, Action::Replace)?;
...
fn buffer_close_by_ids_impl(
  ...
  let (modified_ids, modified_names): (Vec<_>, Vec<_>) = doc_ids
    ...
    .filter_map(|&doc_id| {
      if let Err(CloseError::BufferModified(name)) =
        cx.session.close_document(doc_id, force)
      {
        Some((doc_id, name))
      } else {
        None
  ...
  if let Some(first) = modified_ids.first() {
    let current = doc!(cx.session);
    // If the current document is unmodified, and there are modified
    // documents, switch focus to the first modified doc.
    if !modified_ids.contains(&current.id()) {
      cx.session.switch(*first, Action::Replace);
    }
    bail!(
...
fn buffer_gather_paths_impl(
  session: &mut Session,
  args: &[Cow<str>],
) -> Vec<DocumentId> {
  // No arguments implies current document
  if args.is_empty() {
    let doc_id = view!(session).doc;
    return vec![doc_id];
  }
  ...
  for arg in args {
    let doc_id = session.documents().find_map(|doc| {
...
fn buffer_close(
  ...
  let document_ids = buffer_gather_paths_impl(cx.session, args);
...
fn buffer_gather_others_impl(session: &mut Session) -> Vec<DocumentId> {
  let current_document = &doc!(session).id();
  session
    .documents()
    .map(|doc| doc.id())
...
fn buffer_close_others(
  ...
  let document_ids = buffer_gather_others_impl(cx.session);
  buffer_close_by_ids_impl(cx, &document_ids, false)
}
...
fn buffer_gather_all_impl(session: &mut Session) -> Vec<DocumentId> {
  session.documents().map(|doc| doc.id()).collect()
}
...
fn buffer_close_all(
  ...
  let document_ids = buffer_gather_all_impl(cx.session);
  buffer_close_by_ids_impl(cx, &document_ids, false)
}
...
fn insert_final_newline(doc: &mut Document, view: &mut View) {
  ...
  if line_ending::get_line_ending(&text.slice(..)).is_none() {
    ...
    let insert =
      Transaction::insert(text, &eof, doc.line_ending.as_str().into());
...
fn new_file(
  ...
  cx.session.new_file(Action::Replace);
...
fn format(
  ...
  if let Some(format) = doc.format() {
    let callback =
      make_format_callback(doc.id(), doc.version(), view.id, format, None);
    cx.jobs.callback(callback);
  }
...
fn set_indent_style(
  ...
  if args.is_empty() {
    let style = doc!(cx.session).indent_style;
    cx.session.set_status(match style {
      Tabs => "tabs".to_owned(),
...
fn set_line_ending(
  ...
  if args.is_empty() {
    ...
    cx.session.set_status(match line_ending {
      Crlf => "crlf",
      LF => "line feed",
  ...
  let arg = args.first().context("argument missing")?.to_ascii_lowercase();
...
/// Results in an error if there are modified buffers remaining and sets session
/// error, otherwise returns `Ok(())`. If the current document is unmodified,
/// and there are modified documents, switches focus to one of them.
pub(super) fn buffers_remaining_impl(
  session: &mut Session,
) -> anyhow::Result<()> {
  let (modified_ids, modified_names): (Vec<_>, Vec<_>) = session
    .documents()
    .filter(|doc| doc.is_modified())
    .map(|doc| (doc.id(), doc.display_name()))
    .unzip();
  if let Some(first) = modified_ids.first() {
    let current = doc!(session);
    // If the current document is unmodified, and there are modified
    // documents, switch focus to the first modified doc.
    if !modified_ids.contains(&current.id()) {
      session.switch(*first, Action::Replace);
    }
    bail!(
      "{} unsaved buffer(s) remaining: {:?}",
      modified_names.len(),
...
pub fn write_all_impl(
  ...
  let saves: Vec<_> = cx
    ...
    .filter_map(|id| {
      ...
      let target_view = cx.session.get_synced_view_id(doc.id());
      Some((id, target_view))
    })
    .collect();
  ...
  for (doc_id, target_view) in saves {
    ...
    if config.insert_final_newline {
      insert_final_newline(doc, view_mut!(cx.session, target_view));
    }
    ...
    if fmt.is_none() {
      cx.session.save::<PathBuf>(doc_id, None, force)?;
    }
  }
...
fn quit_all_impl(
  ...
  // close all views
  let views: Vec<_> =
    cx.session.tree.views().map(|(view, _)| view.id).collect();
...
fn cquit(
  ...
  let exit_code =
    args.first().and_then(|code| code.parse::<i32>().ok()).unwrap_or(1);

  cx.session.exit_code = exit_code;
  quit_all_impl(cx, false)
}
...
fn theme(
  ...
  let true_color = cx.session.config.load().true_color || crate::true_color();
  match event {
    PromptEvent::Abort => {
      cx.session.unset_theme_preview();
    },
    PromptEvent::Update => {
      if args.is_empty() {
        ...
        if let Ok(theme) = cx.session.theme_loader.load(theme_name) {
          if !(true_color || theme.is_16_color()) {
            bail!("Unsupported theme: theme requires true color support");
          }
          cx.session.set_theme_preview(theme);
        };
      };
    },
    PromptEvent::Validate => {
      if let Some(theme_name) = args.first() {
        ...
        cx.session.set_theme(theme);
      } else {
        let name = cx.session.theme.name().to_string();

        cx.session.set_status(name);
      }
    },
  };
...
fn yank_joined(
  ...
  let register = cx.session.selected_register.unwrap_or('"');
  yank_joined_impl(cx.session, separator, register);
  Ok(())
}
...
fn yank_joined_to_clipboard(
  ...
  let default_sep = Cow::Borrowed(doc.line_ending.as_str());
  let separator = args.first().unwrap_or(&default_sep);
  yank_joined_impl(cx.session, separator, '+');
  Ok(())
}
...
fn paste_clipboard_after(
  ...
  paste(cx.session, '+', Paste::After, 1);
  Ok(())
}
...
fn paste_clipboard_before(
  ...
  paste(cx.session, '+', Paste::Before, 1);
  Ok(())
}
...
fn replace_selections_with_clipboard(
  ...
  replace_with_yanked_impl(cx.session, '+', 1);
  Ok(())
}
...
fn show_clipboard_provider(
  ...
  cx.session
    .set_status(cx.session.registers.clipboard_provider_name().to_string());
  Ok(())
}
...
fn change_current_directory(
  ...
  let dir = args.first().context("target directory not provided")?.as_ref();
  let dir = helix_stdx::path::expand_tilde(Path::new(dir));

  helix_stdx::env::set_current_working_dir(dir)?;

  cx.session.set_status(format!(
...
fn show_current_directory(
  ...
  if cwd.exists() {
    cx.session.set_status(message);
  } else {
    cx.session.set_error(format!("{} (deleted)", message));
  }
  Ok(())
}
...
fn set_encoding(
  ...
  if let Some(label) = args.first() {
    ...
    cx.session.set_status(encoding);
    Ok(())
  }
}
...
fn get_character_info(
  ...
  cx.session.set_status(format!("\"{printable}\"{unicode}{dec} Hex{hex}"));
...
fn reload(
  ...
  let scrolloff = cx.session.config().scrolloff;
  let (view, doc) = current!(cx.session);
  doc.reload(view, &cx.session.diff_providers).map(|_| {
    view.ensure_cursor_in_view(doc, scrolloff);
  })?;
  if let Some(path) = doc.path() {
    cx.session.language_servers.file_event_handler.file_changed(path.clone());
  }
  Ok(())
}
...
fn reload_all(
  ...
  for (doc_id, view_ids) in docs_view_ids {
    ...
    let view = view_mut!(cx.session, view_ids[0]);

    // Ensure that the view is synced with the document's history.
    view.sync_changes(doc);

    doc.reload(view, &cx.session.diff_providers)?;
    if let Some(path) = doc.path() {
      cx.session.language_servers.file_event_handler.file_changed(path.clone());
    }

    for view_id in view_ids {
      let view = view_mut!(cx.session, view_id);
...
fn update(
  ...
  let (_view, doc) = current!(cx.session);
  if doc.is_modified() {
    write(cx, args, event)
  } else {
    Ok(())
  ...
  if args.is_empty() {
    ...
    let callback = async move {
      let call: job::Callback = Callback::SessionCompositor(Box::new(
        move |_session: &mut Session, compositor: &mut Compositor| {
          let picker =
            ui::Picker::new(commands, (), move |cx, command, _action| {
              execute_lsp_command(
                cx.session,
                language_server_id,
                command.clone(),
...
fn lsp_restart(
  ...
  let session_config = cx.session.config.load();
  let (_view, doc) = current!(cx.session);
  let config = doc
    .language_config()
    .context("LSP not defined for the current document")?;

  cx.session.language_servers.restart(
    config,
    doc.path(),
    &session_config.workspace_lsp_roots,
    session_config.lsp.snippets,
  )?;

  // This collect is needed because refresh_language_server would need to re-borrow session.
  ...
  for document_id in document_ids_to_refresh {
    cx.session.refresh_language_servers(document_id);
  }
...
fn lsp_stop(
  ...
  let ls_shutdown_names = doc!(cx.session)
    .language_servers()
    .map(|ls| ls.name().to_string())
    .collect::<Vec<_>>();

  for ls_name in &ls_shutdown_names {
    cx.session.language_servers.stop(ls_name);

    for doc in cx.session.documents_mut() {
...
fn tree_sitter_scopes(
  ...
  let callback = async move {
    let call: job::Callback = Callback::SessionCompositor(Box::new(
      move |session: &mut Session, compositor: &mut Compositor| {
        let contents = ui::Markdown::new(contents, session.syn_loader.clone());
...
fn tree_sitter_highlight_name(
  ...
  let content = cx.session.theme.scope(highlight.0).to_string();

  let callback = async move {
    let call: job::Callback = Callback::SessionCompositor(Box::new(
      move |session: &mut Session, compositor: &mut Compositor| {
        let content = ui::Markdown::new(content, session.syn_loader.clone());
...
fn vsplit(
  ...
  if args.is_empty() {
    ...
    for arg in args {
      cx.session.open(&PathBuf::from(arg.as_ref()), Action::VerticalSplit)?;
    }
  }
...
fn hsplit(
  ...
  if args.is_empty() {
    ...
    for arg in args {
      cx.session.open(&PathBuf::from(arg.as_ref()), Action::HorizontalSplit)?;
    }
  }
...
fn debug_eval(
  ...
  if let Some(debugger) = cx.session.debugger.as_mut() {
    ...
    let response =
      helix_lsp::block_on(debugger.eval(args.join(" "), Some(frame_id)))?;
    cx.session.set_status(response.result);
  }
  Ok(())
}
...
fn tutor(
  ...
  cx.session.open(&path, Action::Replace)?;
  // Unset path to prevent accidentally saving to the original tutor file.
  doc_mut!(cx.session).set_path(None);
  Ok(())
}

fn abort_goto_line_number_preview(cx: &mut compositor::Context) {
  if let Some(last_selection) = cx.session.last_selection.take() {
    let scrolloff = cx.session.config().scrolloff;
...
fn update_goto_line_number_preview(
  ...
  cx.session.last_selection.get_or_insert_with(|| {
    let (view, doc) = current!(cx.session);
  ...
  goto_line_without_jumplist(cx.session, NonZeroUsize::new(line));
...
pub(super) fn goto_line_number(
  ...
  match event {
    ...
    PromptEvent::Validate => {
      ...
      let last_selection = cx.session.last_selection.take().expect(
        "update_goto_line_number_preview should always set last_selection",
      );
    ...
    PromptEvent::Update if args.is_empty() => {
      abort_goto_line_number_preview(cx)
    },
...
fn get_option(
  ...
  let config = serde_json::json!(cx.session.config().deref());
  let pointer = format!("/{}", key.replace('.', "/"));
  let value = config.pointer(&pointer).ok_or_else(key_error)?;

  cx.session.set_status(value.to_string());
  Ok(())
}
...
fn set_option(
  ...
  let mut config = serde_json::json!(&cx.session.config().deref());
  let pointer = format!("/{}", key.replace('.', "/"));
  ...
  *value = if value.is_string() {
  ...
  cx.session.config_events.0.send(ConfigEvent::Update(config))?;
  Ok(())
}
...
fn language(
  ...
  if args.is_empty() {
    ...
    cx.session.set_status(language.to_string());
    return Ok(());
  }
  ...
  if args[0] == DEFAULT_LANGUAGE_NAME {
    ...
    doc.set_language_by_language_id(&args[0], cx.session.syn_loader.clone())?;
  }
  doc.detect_indent_and_line_ending();

  let id = doc.id();
  cx.session.refresh_language_servers(id);
  let doc = doc_mut!(cx.session);
  let diagnostics = Session::doc_diagnostics(
    &cx.session.language_servers,
    &cx.session.diagnostics,
    doc,
  );
...
fn reflow(
  ...
  let scrolloff = cx.session.config().scrolloff;
  let cfg_text_width: usize = cx.session.config().text_width;
  let (view, doc) = current!(cx.session);
  ...
  let transaction =
    Transaction::change_by_selection(rope, selection, |range| {
      ...
      let reflowed_text =
        helix_core::wrap::reflow_hard_wrap(&fragment, text_width);
    ...
    if let Some(selected_node) = syntax.descendant_for_byte_range(from, to) {
      ...
      let callback = async move {
        let call: job::Callback = Callback::SessionCompositor(Box::new(
          move |session: &mut Session, compositor: &mut Compositor| {
            let contents =
              ui::Markdown::new(contents, session.syn_loader.clone());
            let popup = Popup::new("hover", contents).auto_close(true);
            compositor.replace_or_push("hover", popup);
          },
        ));
...
fn open_config(
  ...
  cx.session.open(&helix_loader::config_file(), Action::Replace)?;
  Ok(())
}
...
fn open_workspace_config(
  ...
  cx.session.open(&helix_loader::workspace_config_file(), Action::Replace)?;
  Ok(())
}
...
fn open_log(
  ...
  cx.session.open(&helix_loader::log_file(), Action::Replace)?;
  Ok(())
}
...
fn refresh_config(
  ...
  cx.session.config_events.0.send(ConfigEvent::Refresh)?;
  Ok(())
}
...
fn run_shell_command(
  ...
  let shell = cx.session.config().shell.clone();
  let args = args.join(" ");
      ...
      move |session: &mut Session, compositor: &mut Compositor| {
        if !output.is_empty() {
          let contents = ui::Markdown::new(
            format!("```sh\n{}\n```", output),
            session.syn_loader.clone(),
          );
          let popup = Popup::new("shell", contents).position(Some(
            helix_core::Position::new(
              session.cursor().0.unwrap_or_default().row,
              2,
            ),
          ));
        ...
        if success {
          session.set_status("Command succeeded");
        } else {
          session.set_error("Command failed");
        }
      },
    ));
    Ok(call)
...
fn reset_diff_change(
  ...
  let text: Tendril =
    diff.diff_base().slice(before_start..before_end).chunks().collect();
...
fn clear_register(
  ...
  if args.is_empty() {
    cx.session.registers.clear();
    cx.session.set_status("All registers cleared");
    return Ok(());
  }
  ...
  if cx.session.registers.remove(register) {
    cx.session.set_status(format!("Register {} cleared", register));
  } else {
    cx.session.set_error(format!("Register {} not found", register));
  }
  Ok(())
}
...
fn redraw(
  ...
  let callback = Box::pin(async move {
    let call: job::Callback =
      job::Callback::SessionCompositor(Box::new(|_session, compositor| {
...
fn move_buffer(
  ...
  if let Err(err) = cx.session.move_path(&old_path, new_path.as_ref()) {
...
pub const TYPABLE_COMMAND_LIST: &[TypableCommand] = &[
    ...
    TypableCommand {
        name: "theme",
        aliases: &[],
        doc: "Change the session theme (show current theme if no name specified).",
        fun: theme,
...
pub static TYPABLE_COMMAND_MAP: Lazy<
  HashMap<&'static str, &'static TypableCommand>,
> = Lazy::new(|| {
...
pub(super) fn command_mode(cx: &mut Context) {
  let mut prompt = Prompt::new(
    ":".into(),
    Some(':'),
    |session: &Session, input: &str| {
      let shellwords = Shellwords::from(input);
      let words = shellwords.words();

      if words.is_empty()
        || (words.len() == 1 && !shellwords.ends_with_whitespace())
      {
        fuzzy_match(
          input,
    ...
    move |cx: &mut compositor::Context, input: &str, event: PromptEvent| {
      ...
      if parts.len() == 1 && parts[0].parse::<usize>().ok().is_some() {
        if let Err(e) =
          typed::goto_line_number(cx, &[Cow::from(parts[0])], event)
        {
      ...
      if let Some(cmd) = typed::TYPABLE_COMMAND_MAP.get(parts[0]) {
        ...
        cx.session.set_error(format!("no such command: '{}'", parts[0]));
      }
    },
  );
  ...
  prompt.recalculate_completion(cx.session);
