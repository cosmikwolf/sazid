...
use crate::{components::session::Session, job::Jobs, ui::picker};
// use crate::ui::picker;
...
impl<'a> Context<'a> {
  ...
  pub fn block_try_flush_writes(&mut self) -> anyhow::Result<()> {
    tokio::task::block_in_place(|| {
      helix_lsp::block_on(self.jobs.finish(self.session, None))
    })?;
    tokio::task::block_in_place(|| {
      helix_lsp::block_on(self.session.flush_writes())
    })?;
    Ok(())
  }
}
...
impl Compositor {
  pub fn new(area: Rect) -> Self {
    Self { layers: Vec::new(), area, last_picker: None, full_redraw: false }
  }
  ...
  pub fn remove(&mut self, id: &'static str) -> Option<Box<dyn Component>> {
    let idx = self.layers.iter().position(|layer| layer.id() == Some(id))?;
  ...
  pub fn handle_event(&mut self, event: &Event, cx: &mut Context) -> bool {
    ...
    if let (Event::Key(key), Some((_, keys))) =
      (event, &mut cx.session.macro_recording)
    {
      keys.push(*key);
    }
  ...
  pub fn cursor(
    ...
    for layer in self.layers.iter().rev() {
      if let (Some(pos), kind) = layer.cursor(area, session) {
  ...
  pub fn has_component(&self, type_name: &str) -> bool {
    self.layers.iter().any(|component| component.type_name() == type_name)
  }
