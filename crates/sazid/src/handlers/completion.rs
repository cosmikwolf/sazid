...
use crate::ui::session::InsertEvent;
...
impl CompletionHandler {
  pub fn new(config: Arc<ArcSwap<Config>>) -> CompletionHandler {
    Self { config, request: None, trigger: None }
  }
}
...
impl helix_event::AsyncHook for CompletionHandler {
  ...
  fn handle_event(
    ...
    self.trigger.map(|trigger| {
      ...
      let timeout = if trigger.kind == TriggerKind::Auto && !cancel {
        self.config.load().session.completion_timeout
      } else {
  ...
  fn finish_debounce(&mut self) {
    ...
    dispatch_blocking(move |session, compositor| {
      request_completion(trigger, rx, session, compositor)
    });
  }
}
...
fn request_completion(
  ...
  if compositor.find::<ui::SessionView>().unwrap().completion.is_some()
  ...
  let ui = compositor.find::<ui::SessionView>().unwrap();
  ...
  tokio::spawn(async move {
    ...
    dispatch(move |session, compositor| {
      show_completion(session, compositor, items, trigger, savepoint)
    })
    .await
  });
}
...
fn show_completion(
  ...
  let completion_area =
    ui.set_completion(session, savepoint, items, trigger.pos, size);
  let signature_help_area = compositor
    .find_id::<Popup<SignatureHelp>>(SignatureHelp::ID)
    .map(|signature_help| signature_help.area(size, session));
...
fn update_completions(cx: &mut commands::Context, c: Option<char>) {
  cx.callback.push(Box::new(move |compositor, cx| {
    let session_view = compositor.find::<ui::SessionView>().unwrap();
    if let Some(completion) = &mut session_view.completion {
      completion.update_filter(c);
      if completion.is_empty() {
        session_view.clear_completion(cx.session);
        ...
        if c.is_some() {
          trigger_auto_completion(
            &cx.session.handlers.completions,
            cx.session,
            false,
          );
        }
...
fn completion_post_command_hook(
  ...
  if cx.session.mode == Mode::Insert {
    if cx.session.last_completion.is_some() {
      match command {
        MappableCommand::Static {
          name: "delete_word_forward" | "delete_char_forward" | "completion",
          ..
        } => (),
        MappableCommand::Static { name: "delete_char_backward", .. } => {
...
pub(super) fn register_hooks(handlers: &Handlers) {
  ...
  register_hook!(move |event: &mut PostCommand<'_, '_>| {
    completion_post_command_hook(&tx, event)
  });
  ...
  register_hook!(move |event: &mut OnModeSwitch<'_, '_>| {
    if event.old_mode == Mode::Insert {
      ...
      trigger_auto_completion(&tx, event.cx.session, false)
    }
    Ok(())
  });
  ...
  register_hook!(move |event: &mut PostInsertChar<'_, '_>| {
    if event.cx.session.last_completion.is_some() {
