...
impl SignatureHelpHandler {
  pub fn new() -> SignatureHelpHandler {
    SignatureHelpHandler { trigger: None, state: State::Closed }
  }
}
...
impl helix_event::AsyncHook for SignatureHelpHandler {
  ...
  fn finish_debounce(&mut self) {
    ...
    job::dispatch_blocking(move |session, _| {
      request_signature_help(session, invocation, rx)
    })
  }
}
...
pub fn request_signature_help(
  ...
  tokio::spawn(async move {
    match cancelable_future(future, cancel).await {
      Some(Ok(res)) => {
        job::dispatch(move |session, compositor| {
          show_signature_help(session, compositor, invoked, res)
        })
        .await
      },
...
pub fn show_signature_help(
  ...
  // annoyance, see https://github.com/helix-session/helix/issues/3112
  ...
  if invoked == SignatureHelpInvoked::Automatic && session.mode != Mode::Insert
  {
    return;
  }
  ...
  let response = match response {
    ...
    _ => {
      send_blocking(
        &session.handlers.signature_hints,
  ...
  let active_param_range = || -> Option<(usize, usize)> {
    let param_idx =
      signature.active_parameter.or(response.active_parameter).unwrap_or(0)
        as usize;
  ...
  if compositor
    ...
    .completion
    .as_mut()
    .map(|completion| completion.area(size, session))
    .filter(|area| area.intersects(popup.area(size, session)))
    .is_some()
  {
    return;
  }
...
fn signature_help_post_insert_char_hook(
  ...
  if !cx.session.config().lsp.auto_signature_help {
    return Ok(());
  }
...
pub(super) fn register_hooks(handlers: &Handlers) {
  ...
  register_hook!(move |event: &mut OnModeSwitch<'_, '_>| {
    match (event.old_mode, event.new_mode) {
      ...
      (_, Mode::Insert) => {
        if event.cx.session.config().lsp.auto_signature_help {
          send_blocking(&tx, SignatureHelpEvent::Trigger);
        }
      },
      _ => (),
