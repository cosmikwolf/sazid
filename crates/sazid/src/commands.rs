...
use helix_core::{
  ...
  visual_offset_from_block, Deletion, LineEnding, Position, Range, Rope,
...
impl<'a> Context<'a> {
  ...
  pub fn push_layer(&mut self, component: Box<dyn Component>) {
    self.callback.push(Box::new(|compositor: &mut Compositor, _| {
      compositor.push(component)
    }));
  }

  /// Call `replace_or_push` on the Compositor
  pub fn replace_or_push_layer<T: Component>(
    &mut self,
    id: &'static str,
    component: T,
  ) {
    self.callback.push(Box::new(move |compositor: &mut Compositor, _| {
  ...
  pub fn callback<T, F>(
    &mut self,
    call: impl Future<Output = helix_lsp::Result<serde_json::Value>>
      + 'static
      + Send,
    callback: F,
  ) where
    T: for<'de> serde::Deserialize<'de> + Send + 'static,
    F: FnOnce(&mut Session, &mut Compositor, T) + Send + 'static,
  {
  ...
  F: FnOnce(&mut Session, &mut Compositor, T) + Send + 'static,
{
  Box::pin(async move {
    ...
    let call: job::Callback = Callback::SessionCompositor(Box::new(
      move |session: &mut Session, compositor: &mut Compositor| {
        callback(session, compositor, response)
      },
    ));
    Ok(call)
  })
...
pub enum MappableCommand {
  Typable { name: String, args: Vec<String>, doc: String },
  ...
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      MappableCommand::Typable { name, args, .. } => {
        f.debug_tuple("MappableCommand").field(name).field(args).finish()
      },
    }
  }
}
...
impl std::str::FromStr for MappableCommand {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    if let Some(suffix) = s.strip_prefix(':') {
      ...
      let args = typable_command.map(|s| s.to_owned()).collect::<Vec<String>>();
...
impl PartialEq for MappableCommand {
  fn eq(&self, other: &Self) -> bool {
    match (self, other) {
      (
        MappableCommand::Typable { name: first_name, args: first_args, .. },
        MappableCommand::Typable {
          name: second_name, args: second_args, ..
        },
      ) => first_name == second_name && first_args == second_args,
      (
        MappableCommand::Static { name: first_name, .. },
        MappableCommand::Static { name: second_name, .. },
...
fn move_impl(
  ...
  let count = cx.count();
  let (view, doc) = current!(cx.session);
...
fn goto_line_end(cx: &mut Context) {
  ...
  goto_line_end_impl(
    view,
    doc,
    if cx.session.mode == Mode::Select {
      Movement::Extend
    } else {
      Movement::Move
    },
...
fn goto_next_buffer(cx: &mut Context) {
  goto_buffer(cx.session, Direction::Forward);
}

fn goto_previous_buffer(cx: &mut Context) {
  goto_buffer(cx.session, Direction::Backward);
}

fn goto_buffer(session: &mut Session, direction: Direction) {
  let current = view!(session).doc;

  let id = match direction {
    Direction::Forward => {
      let iter = session.documents.keys();
      let mut iter = iter.skip_while(|id| *id != &current);
      iter.next(); // skip current item
      iter.next().or_else(|| session.documents.keys().next())
    },
    Direction::Backward => {
      let iter = session.documents.keys();
      let mut iter = iter.rev().skip_while(|id| *id != &current);
      iter.next(); // skip current item
      iter.next().or_else(|| session.documents.keys().next_back())
    },
  }
  .unwrap();
  ...
  session.switch(id, Action::Replace);
}
...
fn trim_selections(cx: &mut Context) {
  ...
  let ranges: SmallVec<[Range; 1]> = doc
    ...
    .filter_map(|range| {
      if range.is_empty()
        || range.slice(text).chars().all(|ch| ch.is_whitespace())
      {
        return None;
      }
      let mut start = range.from();
      let mut end = range.to();
      start = movement::skip_while(text, start, |x| x.is_whitespace())
        .unwrap_or(start);
      end = movement::backwards_skip_while(text, end, |x| x.is_whitespace())
        .unwrap_or(end);
...
fn align_selections(cx: &mut Context) {
  ...
  for range in selection {
    ...
    if coords.row != anchor_coords.row {
      cx.session.set_error("align cannot work with multi line selections");
      return;
    }
...
fn goto_window(cx: &mut Context, align: Align) {
  ...
  let visual_line = match align {
    ...
    Align::Bottom => {
      view.offset.vertical_offset
        + last_visual_line.saturating_sub(scrolloff + count)
    },
  };
  let visual_line =
    visual_line.max(view.offset.vertical_offset + scrolloff).min(
  ...
  let text = doc.text().slice(..);
  let selection = doc.selection(view.id).clone().transform(|range| {
    range.put_cursor(text, pos, cx.session.mode == Mode::Select)
  });
  doc.set_selection(view.id, selection);
}
...
{
  let count = cx.count();
  let motion = move |session: &mut Session| {
    let (view, doc) = current!(session);
    let text = doc.text().slice(..);
    let behavior = if session.mode == Mode::Select {
      Movement::Extend
    } else {
      Movement::Move
    };
  ...
  cx.session.apply_motion(motion)
}
...
fn goto_file_start(cx: &mut Context) {
  if cx.count.is_some() {
    ...
    let text = doc.text().slice(..);
    let selection = doc.selection(view.id).clone().transform(|range| {
      range.put_cursor(text, 0, cx.session.mode == Mode::Select)
    });
    push_jump(view, doc);
    doc.set_selection(view.id, selection);
  }
...
fn goto_file_impl(cx: &mut Context, action: Action) {
  let (view, doc) = current_ref!(cx.session);
  let text = doc.text();
  ...
  for sel in paths {
    ...
    if path.is_dir() {
      let picker = ui::file_picker(path.into(), &cx.session.config());
      cx.push_layer(Box::new(overlaid(picker)));
    } else if let Err(e) = cx.session.open(path, action) {
      cx.session.set_error(format!("Open file failed: {:?}", e));
    }
  }
}
  ...
  let selection = doc.selection(view.id).clone().transform(|range| {
    ...
    let find_on_line = match direction {
      ...
      Direction::Backward => {
        ...
        let line =
          cursor_line as isize - (count as isize - 1 + on_edge as isize);
        if line <= 0 {
          return range;
        } else {
          line as usize
    ...
    let pos = match (direction, inclusive) {
      ...
      (Direction::Forward, false) => {
        line_end_char_index(&text, find_on_line) - 1
      },
...
fn find_char(
  ...
  cx.on_next_key(move |cx, event| {
    let ch = match event {
      ...
      KeyEvent { code: KeyCode::Char(ch), .. } => ch,
      _ => return,
    };
    let motion = move |session: &mut Session| {
      match direction {
  ...
  F: Fn(RopeSlice, M, usize, usize, bool) -> Option<usize> + 'static,
{
  ...
  let selection = doc.selection(view.id).clone().transform(|range| {
    ...
    let search_start_pos =
      if range.anchor < range.head { range.head - 1 } else { range.head };

    search_fn(text, char_matcher, search_start_pos, count, inclusive).map_or(
      range,
      |pos| {
        if extend {
          range.put_cursor(text, pos, true)
...
fn repeat_last_motion(cx: &mut Context) {
  cx.session.repeat_last_motion(cx.count())
}
...
fn replace(cx: &mut Context) {
  ...
  cx.on_next_key(move |cx, event| {
    ...
    let ch: Option<&str> = match event {
      ...
      KeyEvent { code: KeyCode::Enter, .. } => Some(doc.line_ending.as_str()),
      KeyEvent { code: KeyCode::Tab, .. } => Some("\t"),
      _ => None,
    };
...
pub fn scroll(
  ...
  if sync_cursor {
    let movement = match cx.session.mode {
  ...
  let anchor =
    if cx.session.mode == Mode::Select { range.anchor } else { head };
...
fn copy_selection_on_line(cx: &mut Context, direction: Direction) {
  ...
  for range in selection.iter() {
    ...
    while sels < count {
      ...
      let anchor = pos_at_visual_coords(
        text,
        Position::new(anchor_row, anchor_pos.col),
        tab_width,
      );
      let head = pos_at_visual_coords(
        text,
...
fn split_selection(cx: &mut Context) {
  ...
  ui::regex_prompt(
    ...
    move |cx, regex, event| {
      ...
      let text = doc.text().slice(..);
      let selection =
        selection::split_on_matches(text, doc.selection(view.id), &regex);
      doc.set_selection(view.id, selection);
    },
  );
}
...
fn search_impl(
  ...
  let start = match direction {
    Direction::Forward => text.char_to_byte(
      graphemes::ensure_grapheme_boundary_next(text, selection.primary().to()),
    ),
    Direction::Backward => {
      text.char_to_byte(graphemes::ensure_grapheme_boundary_prev(
        text,
  ...
  let doc = doc!(session).text().slice(..);
  ...
  if mat.is_none() {
    ...
    if show_warnings {
      if wrap_around && mat.is_some() {
        session.set_status("Wrapped around document");
      } else {
        session.set_error("No more matches");
      }
    }
  }
...
fn search_completions(cx: &mut Context, reg: Option<char>) -> Vec<String> {
  let mut items = reg
    .and_then(|reg| cx.session.registers.read(reg, cx.session))
...
fn searcher(cx: &mut Context, direction: Direction) {
  ...
  ui::regex_prompt(
    ...
    "search:".into(),
    Some(reg),
    move |_session: &Session, input: &str| {
      completions
        .iter()
    ...
    move |cx, regex, event| {
      if event == PromptEvent::Validate {
        cx.session.registers.last_search_register = reg;
...
fn search_next_or_prev_impl(
  ...
  let count = cx.count();
  let register =
    cx.register.unwrap_or(cx.session.registers.last_search_register);
  let config = cx.session.config();
  let scrolloff = config.scrolloff;
  if let Some(query) = cx.session.registers.first(register, cx.session) {
    ...
    if let Ok(regex) = rope::RegexBuilder::new()
      .syntax(
        rope::Config::new().case_insensitive(case_insensitive).multi_line(true),
      )
      .build(&query)
    {
      for _ in 0..count {
...
fn search_selection(cx: &mut Context) {
  ...
  match cx.session.registers.push(register, regex) {
    Ok(_) => {
      cx.session.registers.last_search_register = register;
      cx.session.set_status(msg)
    },
    Err(err) => cx.session.set_error(err.to_string()),
  }
}
...
fn make_search_word_bounded(cx: &mut Context) {
  ...
  let register =
    cx.register.unwrap_or(cx.session.registers.last_search_register);
  let regex = match cx.session.registers.first(register, cx.session) {
    Some(regex) => regex,
    None => return,
  };
  ...
  let mut new_regex = String::with_capacity(
    regex.len()
      + if start_anchored { 0 } else { 2 }
      + if end_anchored { 0 } else { 2 },
  );
  ...
  match cx.session.registers.push(register, new_regex) {
    Ok(_) => {
      cx.session.registers.last_search_register = register;
      cx.session.set_status(msg)
    },
...
//       Self { path: path.to_path_buf(), line_num }
//     }
//   }
//
...
//         .to_string_lossy()
//         .into_owned();
//       if current_path.as_ref().map(|p| p == &self.path).unwrap_or(false) {
...
//       cx.session.registers.last_search_register = reg;
//
//       let current_path = doc_mut!(cx.session).path().cloned();
...
//
//       if let Ok(matcher) =
//         RegexMatcherBuilder::new().case_smart(smart_case).build(input)
//       {
//         let search_root = helix_stdx::env::current_working_dir();
//         if !search_root.exists() {
//           cx.session.set_error("Current working directory does not exist");
//           return;
//         }
//
...
//         let absolute_root =
//           search_root.canonicalize().unwrap_or_else(|_| search_root.clone());
...
//                     let call = move |_: &mut Session, compositor: &mut Compositor| {
//                         let picker = Picker::with_stream(
//                             picker,
//                             injector,
//                             move |cx, FileResult { path, line_num }, action| {
//                                 let doc = match cx.session.open(path, action) {
//                                     Ok(id) => doc_mut!(cx.session, &id),
//                                     Err(e) => {
//                                         cx.session.set_error(format!(
//                                             "Failed to open file '{}': {}",
//                                             path.display(),
//                                             e
//                                         ));
...
//                                 let view = view_mut!(cx.session);
//                                 let text = doc.text();
//                                 if line_num >= text.len_lines() {
//                                     cx.session.set_error(
...
//                         )
//                         .with_preview(
//                             |_session, FileResult { path, line_num }| {
//                                 Some((path.clone().into(), Some((*line_num, *line_num))))
//                             },
//                         );
//                         compositor.push(Box::new(overlaid(picker)))
//                     };
//                     Ok(Callback::SessionCompositor(Box::new(call)))
//                 })
//       } else {
...
fn shrink_to_line_bounds(cx: &mut Context) {
  ...
  doc.set_selection(
    ...
    doc.selection(view.id).clone().transform(|range| {
      ...
      // the session, this will correspond to the cursor being on
...
fn delete_selection_impl(cx: &mut Context, op: Operation) {
  ...
  if cx.register != Some('_') {
    ...
    if let Err(err) = cx.session.registers.write(reg_name, values) {
      cx.session.set_error(err.to_string());
      return;
    }
  };
  ...
  // then delete
  let transaction =
    Transaction::delete_by_selection(doc.text(), selection, |range| {
      (range.from(), range.to())
    });
  doc.apply(&transaction, view.id);
...
fn delete_by_selection_insert_mode(
  ...
  let mut transaction = Transaction::delete_by_selection(
    doc.text(),
...
fn flip_selections(cx: &mut Context) {
  ...
  let selection =
    doc.selection(view.id).clone().transform(|range| range.flip());
  doc.set_selection(view.id, selection);
}
...
fn file_picker(cx: &mut Context) {
  ...
  if !root.exists() {
    cx.session.set_error("Workspace directory does not exist");
    return;
  }
  let picker = ui::file_picker(root, &cx.session.config());
  cx.push_layer(Box::new(overlaid(picker)));
}

fn file_picker_in_current_buffer_directory(cx: &mut Context) {
  let doc_dir = doc!(cx.session)
    .path()
  ...
  let path = match doc_dir {
    Some(path) => path,
    None => {
      cx.session.set_error("current buffer has no path or parent");
      return;
    },
  };

  let picker = ui::file_picker(path, &cx.session.config());
  cx.push_layer(Box::new(overlaid(picker)));
}
...
fn file_picker_in_current_directory(cx: &mut Context) {
  ...
  let picker = ui::file_picker(cwd, &cx.session.config());
  cx.push_layer(Box::new(overlaid(picker)));
}

fn buffer_picker(cx: &mut Context) {
  let current = view!(cx.session).doc;
  ...
  impl ui::menu::Item for BufferMeta {
    ...
    fn format(&self, _data: &Self::Data) -> Row {
      let path = self.path.as_deref().map(helix_stdx::path::get_relative_path);
  ...
  let mut items =
    cx.session.documents.values().map(new_meta).collect::<Vec<BufferMeta>>();

  // mru
  items.sort_unstable_by_key(|item| std::cmp::Reverse(item.focused_at));

  let picker = Picker::new(items, (), |cx, meta, action| {
    cx.session.switch(meta.id, action);
  })
  .with_preview(|session, meta| {
    let doc = &session.documents.get(&meta.id)?;
    let &view_id = doc.selections().keys().next()?;
    let line =
      doc.selection(view_id).primary().cursor_line(doc.text().slice(..));
...
fn jumplist_picker(cx: &mut Context) {
  ...
  for (view, _) in cx.session.tree.views_mut() {
    for doc_id in view.jumps.iter().map(|e| e.0).collect::<Vec<_>>().iter() {
      let doc = doc_mut!(cx.session, doc_id);
      view.sync_changes(doc);
    }
  }

  let new_meta = |view: &View, doc_id: DocumentId, selection: Selection| {
    let doc = &cx.session.documents.get(&doc_id);
  ...
  let picker = Picker::new(
    ...
    |cx, meta, action| {
      cx.session.switch(meta.id, action);
      let config = cx.session.config();
      let (view, doc) = (view_mut!(cx.session), doc_mut!(cx.session, &meta.id));
    ...
    match self {
      MappableCommand::Typable { doc, name, .. } => {
        match keymap.get(name as &String) {
          Some(bindings) => {
            format!("{} ({}) [:{}]", doc, fmt_binding(bindings), name).into()
          },
...
pub fn command_palette(cx: &mut Context) {
  ...
  cx.callback.push(Box::new(
    move |compositor: &mut Compositor, cx: &mut compositor::Context| {
      let keymap = compositor.find::<ui::SessionView>().unwrap().keymaps.map()
        [&cx.session.mode]
        .reverse_map();
      ...
      let picker =
        Picker::new(commands, keymap, move |cx, command, _action| {
          let mut ctx = Context {
            register,
            count,
            session: cx.session,
          ...
          let focus = view!(ctx.session).id;

          command.execute(&mut ctx);

          if ctx.session.tree.contains(focus) {
            let config = ctx.session.config();
            let mode = ctx.session.mode();
            let view = view_mut!(ctx.session, focus);
            let doc = doc_mut!(ctx.session, &view.doc);
...
fn last_picker(cx: &mut Context) {
  ...
  cx.callback.push(Box::new(|compositor, cx| {
    if let Some(picker) = compositor.last_picker.take() {
      ...
      cx.session.set_error("no last picker")
    }
  }));
}
...
fn insert_with_indent(cx: &mut Context, cursor_fallback: IndentFallbackPos) {
  ...
  let mut transaction =
    Transaction::change_by_selection(contents, selection, |range| {
      let cursor_line = range.cursor_line(text);
      ...
      if line_end_char_index(&text, cursor_line) == cursor_line_start {
        ...
        ranges.push(range.put_cursor(
          text,
          pos + offs,
          cx.session.mode == Mode::Select,
        ));
...
async fn make_format_callback(
  ...
  let call: job::Callback = Callback::Session(Box::new(move |session| {
    if !session.documents.contains_key(&doc_id)
      || !session.tree.contains(view_id)
    {
      return;
    }

    let scrolloff = session.config().scrolloff;
    let doc = doc_mut!(session, &doc_id);
    let view = view_mut!(session, view_id);
    ...
    if let Some((path, force)) = write {
      let id = doc.id();
      if let Err(err) = session.save(id, path, force) {
        session.set_error(format!("Error saving: {}", err));
      }
    }
  }));
  ...
  let mut transaction =
    Transaction::change_by_selection(contents, selection, |range| {
      ...
      let (line_end_index, line_end_offset_width) = if new_line == 0 {
        ...
        (line_end_char_index(&text, line_num), doc.line_ending.len_chars())
      };
...
fn goto_line(cx: &mut Context) {
  if cx.count.is_some() {
    ...
    goto_line_without_jumplist(cx.session, cx.count);
  }
}
...
fn goto_line_without_jumplist(
  ...
  if let Some(count) = count {
    ...
    let selection = doc.selection(view.id).clone().transform(|range| {
      range.put_cursor(text, pos, session.mode == Mode::Select)
    });
...
fn goto_last_accessed_file(cx: &mut Context) {
  ...
  if let Some(alt) = view.docs_access_history.pop() {
    cx.session.switch(alt, Action::Replace);
  } else {
    cx.session.set_error("no last accessed buffer")
  }
}
...
fn goto_last_modified_file(cx: &mut Context) {
  let view = view!(cx.session);
  let alternate_file =
    view.last_modified_docs.into_iter().flatten().find(|&id| id != view.doc);
  if let Some(alt) = alternate_file {
    cx.session.switch(alt, Action::Replace);
  } else {
    cx.session.set_error("no last modified buffer")
  }
}
...
//       let idx = if reverse { diff.len().saturating_sub(1) } else { 0 };
//       diff.nth_hunk(idx)
//     };
//     if hunk != Hunk::NONE {
...
//       diff_handle
//     } else {
//       session.set_status("Diff is not available in current buffer");
//       return;
//     };
//
//     let selection = doc.selection(view.id).clone().transform(|range| {
...
//         Direction::Backward => {
//           diff.prev_hunk(cursor_line).map(|idx| idx.saturating_sub(count))
//         },
//       };
...
pub mod insert {
  ...
  pub fn insert_char(cx: &mut Context, c: char) {
    ...
    let text = doc.text();
    let selection = doc.selection(view.id);
    let auto_pairs = doc.auto_pairs(cx.session);
  ...
  pub fn smart_tab(cx: &mut Context) {
    ...
    if matches!(
      ...
      let cursors_after_whitespace =
        doc.selection(view_id).ranges().iter().all(|range| {
...
fn undo(cx: &mut Context) {
  ...
  for _ in 0..count {
    if !doc.undo(view) {
      cx.session.set_status("Already at oldest change");
      break;
    }
  }
}
...
fn redo(cx: &mut Context) {
  ...
  for _ in 0..count {
    if !doc.redo(view) {
      cx.session.set_status("Already at newest change");
      break;
    }
  }
}
...
fn yank(cx: &mut Context) {
  yank_impl(cx.session, cx.register.unwrap_or('"'));
  exit_select_mode(cx);
}

fn yank_to_clipboard(cx: &mut Context) {
  yank_impl(cx.session, '+');
  exit_select_mode(cx);
}
...
fn yank_impl(session: &mut Session, register: char) {
  let (view, doc) = current!(session);
  let text = doc.text().slice(..);

  let values: Vec<String> =
    doc.selection(view.id).fragments(text).map(Cow::into_owned).collect();
  let selections = values.len();

  match session.registers.write(register, values) {
    Ok(_) => session.set_status(format!(
      "yanked {selections} selection{} to register {register}",
      if selections == 1 { "" } else { "s" }
    )),
    Err(err) => session.set_error(err.to_string()),
  }
}

fn yank_joined_impl(session: &mut Session, separator: &str, register: char) {
  let (view, doc) = current!(session);
  let text = doc.text().slice(..);
  ...
  let joined =
    selection.fragments(text).fold(String::new(), |mut acc, fragment| {
      if !acc.is_empty() {
  ...
  match session.registers.write(register, vec![joined]) {
    Ok(_) => session.set_status(format!(
...
fn yank_joined(cx: &mut Context) {
  let separator = doc!(cx.session).line_ending.as_str();
  yank_joined_impl(cx.session, separator, cx.register.unwrap_or('"'));
  exit_select_mode(cx);
}

fn yank_joined_to_clipboard(cx: &mut Context) {
  let line_ending = doc!(cx.session).line_ending;
  yank_joined_impl(cx.session, line_ending.as_str(), '+');
  exit_select_mode(cx);
}
...
fn yank_primary_selection_impl(session: &mut Session, register: char) {
  let (view, doc) = current!(session);
  let text = doc.text().slice(..);

  let selection = doc.selection(view.id).primary().fragment(text).to_string();

  match session.registers.write(register, vec![selection]) {
    Ok(_) => session
      .set_status(format!("yanked primary selection to register {register}",)),
    Err(err) => session.set_error(err.to_string()),
  }
}

fn yank_main_selection_to_clipboard(cx: &mut Context) {
  yank_primary_selection_impl(cx.session, '+');
  exit_select_mode(cx);
}
...
fn paste_impl(
  ...
  let repeat = std::iter::repeat(
    ...
    values.last().map(|value| Tendril::from(value.repeat(count))).unwrap(),
  );

  // if any of values ends with a line ending, it's linewise paste
  let linewise =
    values.iter().any(|value| get_line_ending_of_str(value).is_some());
  ...
  let mut transaction =
    Transaction::change_by_selection(text, selection, |range| {
      ...
      let new_range = Range::new(anchor, anchor + value_len)
...
pub(crate) fn paste_bracketed_value(cx: &mut Context, contents: String) {
  let count = cx.count();
  let paste = match cx.session.mode {
    Mode::Insert | Mode::Select => Paste::Cursor,
    Mode::Normal => Paste::Before,
  };
  let (view, doc) = current!(cx.session);
  paste_impl(&[contents], doc, view, paste, count, cx.session.mode);
  exit_select_mode(cx);
}

fn paste_clipboard_after(cx: &mut Context) {
  paste(cx.session, '+', Paste::After, cx.count());
  exit_select_mode(cx);
}

fn paste_clipboard_before(cx: &mut Context) {
  paste(cx.session, '+', Paste::Before, cx.count());
  exit_select_mode(cx);
}
...
fn replace_with_yanked(cx: &mut Context) {
  replace_with_yanked_impl(cx.session, cx.register.unwrap_or('"'), cx.count());
  exit_select_mode(cx);
}

fn replace_with_yanked_impl(
  session: &mut Session,
  register: char,
  count: usize,
) {
  let Some(values) =
    session.registers.read(register, session).filter(|values| values.len() > 0)
  else {
    return;
  };
...
fn replace_selections_with_clipboard(cx: &mut Context) {
  replace_with_yanked_impl(cx.session, '+', cx.count());
  exit_select_mode(cx);
}
...
fn paste(session: &mut Session, register: char, pos: Paste, count: usize) {
  let Some(values) = session.registers.read(register, session) else {
    return;
  };
  let values: Vec<_> = values.map(|value| value.to_string()).collect();

  let (view, doc) = current!(session);
  paste_impl(&values, doc, view, pos, count, session.mode);
}

fn paste_after(cx: &mut Context) {
  paste(cx.session, cx.register.unwrap_or('"'), Paste::After, cx.count());
  exit_select_mode(cx);
}

fn paste_before(cx: &mut Context) {
  paste(cx.session, cx.register.unwrap_or('"'), Paste::Before, cx.count());
  exit_select_mode(cx);
}
...
fn indent(cx: &mut Context) {
  ...
  let transaction = Transaction::change(
    ...
    lines.into_iter().filter_map(|line| {
      let is_blank =
        doc.text().line(line).chunks().all(|s| s.trim().is_empty());
      if is_blank {
        return None;
      }
  ...
  for selection in doc.selection(view.id) {
    ...
    for line in lines {
      ...
      let mut end = text.line_to_char(line + 1);
        ...
        skip_while(slice, end, |ch| matches!(ch, ' ' | '\t')).unwrap_or(end);
...
fn keep_or_remove_selections_impl(cx: &mut Context, remove: bool) {
  ...
  ui::regex_prompt(
    ...
    move |cx, regex, event| {
      ...
      if let Some(selection) = selection::keep_or_remove_matches(
        text,
        doc.selection(view.id),
        &regex,
        remove,
...
fn remove_primary_selection(cx: &mut Context) {
  ...
  let selection = doc.selection(view.id);
  if selection.len() == 1 {
    cx.session.set_error("no selections remaining");
    return;
  }
...
pub fn completion(cx: &mut Context) {
  ...
  cx.session.handlers.trigger_completions(cursor, doc.id(), view.id);
}
...
fn expand_selection(cx: &mut Context) {
  let motion = |session: &mut Session| {
    let (view, doc) = current!(session);
    ...
    if let Some(syntax) = doc.syntax() {
      ...
      let selection =
        object::expand_selection(syntax, text, current_selection.clone());
...
fn shrink_selection(cx: &mut Context) {
  let motion = |session: &mut Session| {
    ...
    if let Some(syntax) = doc.syntax() {
      let text = doc.text().slice(..);
      let selection =
        object::shrink_selection(syntax, text, current_selection.clone());
      doc.set_selection(view.id, selection);
    }
  };
  cx.session.apply_motion(motion);
  ...
  F: Fn(Node) -> Option<Node>,
{
  let motion = |session: &mut Session| {
    ...
    if let Some(syntax) = doc.syntax() {
      let text = doc.text().slice(..);
      let current_selection = doc.selection(view.id);
      let selection = object::select_sibling(
        syntax,
        text,
...
fn match_brackets(cx: &mut Context) {
  let (view, doc) = current!(cx.session);
  let is_select = cx.session.mode == Mode::Select;
  let text = doc.text();
  let text_slice = text.slice(..);
...
fn jump_forward(cx: &mut Context) {
  ...
  if let Some((id, selection)) = view.jumps.forward(count) {
    ...
    let (view, doc) = current!(cx.session); // refetch doc
...
fn save_selection(cx: &mut Context) {
  let (view, doc) = current!(cx.session);
  push_jump(view, doc);
  cx.session.set_status("Selection saved to jumplist");
}

fn rotate_view(cx: &mut Context) {
  cx.session.focus_next()
}

fn rotate_view_reverse(cx: &mut Context) {
  cx.session.focus_prev()
}

fn jump_view_right(cx: &mut Context) {
  cx.session.focus_direction(tree::Direction::Right)
}

fn jump_view_left(cx: &mut Context) {
  cx.session.focus_direction(tree::Direction::Left)
}

fn jump_view_up(cx: &mut Context) {
  cx.session.focus_direction(tree::Direction::Up)
}

fn jump_view_down(cx: &mut Context) {
  cx.session.focus_direction(tree::Direction::Down)
}

fn swap_view_right(cx: &mut Context) {
  cx.session.swap_split_in_direction(tree::Direction::Right)
}

fn swap_view_left(cx: &mut Context) {
  cx.session.swap_split_in_direction(tree::Direction::Left)
}

fn swap_view_up(cx: &mut Context) {
  cx.session.swap_split_in_direction(tree::Direction::Up)
}

fn swap_view_down(cx: &mut Context) {
  cx.session.swap_split_in_direction(tree::Direction::Down)
}

fn transpose_view(cx: &mut Context) {
  cx.session.transpose_view()
}
...
fn split(session: &mut Session, action: Action) {
  let (view, doc) = current!(session);
  let id = doc.id();
  let selection = doc.selection(view.id).clone();
  let offset = view.offset;

  session.switch(id, action);
...
fn hsplit(cx: &mut Context) {
  split(cx.session, Action::HorizontalSplit);
}

fn hsplit_new(cx: &mut Context) {
  cx.session.new_file(Action::HorizontalSplit);
}

fn vsplit(cx: &mut Context) {
  split(cx.session, Action::VerticalSplit);
}

fn vsplit_new(cx: &mut Context) {
  cx.session.new_file(Action::VerticalSplit);
}

fn wclose(cx: &mut Context) {
  if cx.session.tree.views().count() == 1 {
    if let Err(err) = typed::buffers_remaining_impl(cx.session) {
      cx.session.set_error(err.to_string());
      return;
    }
  }
  let view_id = view!(cx.session).id;
  // close current split
  cx.session.close(view_id);
}

fn wonly(cx: &mut Context) {
  let views =
    cx.session.tree.views().map(|(v, focus)| (v.id, focus)).collect::<Vec<_>>();
...
fn select_register(cx: &mut Context) {
  cx.session.autoinfo = Some(Info::from_registers(&cx.session.registers));
  cx.on_next_key(move |cx, event| {
    if let Some(ch) = event.char() {
      cx.session.autoinfo = None;
      cx.session.selected_register = Some(ch);
    }
  })
}
...
fn insert_register(cx: &mut Context) {
  ...
  cx.on_next_key(move |cx, event| {
    if let Some(ch) = event.char() {
      cx.session.autoinfo = None;
      cx.register = Some(ch);
      paste(cx.session, cx.register.unwrap_or('"'), Paste::Cursor, cx.count());
    }
  })
}
...
fn align_view_middle(cx: &mut Context) {
  ...
  view.offset.horizontal_offset =
    pos.col.saturating_sub((view.inner_area(doc).width as usize) / 2);
}
...
fn goto_ts_object_impl(
  ...
  let motion = move |session: &mut Session| {
    ...
    if let Some((lang_config, syntax)) = doc.language_config().zip(doc.syntax())
    {
      ...
      session.set_status("Syntax-tree is not available in current buffer");
    }
  };
  cx.session.apply_motion(motion);
}
...
fn select_textobject(cx: &mut Context, objtype: textobject::TextObject) {
  ...
  cx.on_next_key(move |cx, event| {
    cx.session.autoinfo = None;
    if let Some(ch) = event.char() {
      let textobject = move |session: &mut Session| {
        let (view, doc) = current!(session);
        let text = doc.text().slice(..);

        let textobject_treesitter = |obj_name: &str, range: Range| -> Range {
          let (lang_config, syntax) =
            match doc.language_config().zip(doc.syntax()) {
              Some(t) => t,
              None => return range,
            };
        ...
        let textobject_change = |range: Range| -> Range {
          ...
          let line = range.cursor_line(text);
          let hunk_idx =
            if let Some(hunk_idx) = diff.hunk_at(line as u32, false) {
              hunk_idx
            } else {
              return range;
            };
      ...
      cx.session.apply_motion(textobject);
    }
  });
  ...
  cx.session.autoinfo = Some(Info::new(title, &help_text));
}
...
fn surround_replace(cx: &mut Context) {
  ...
  cx.on_next_key(move |cx, event| {
    ...
    let change_pos =
      match surround::get_surround_pos(text, selection, surround_ch, count) {
        Ok(c) => c,
        Err(err) => {
          cx.session.set_error(err.to_string());
          return;
...
fn shell_keep_pipe(cx: &mut Context) {
  ui::prompt(
    ...
    move |cx, input: &str, event: PromptEvent| {
      let shell = &cx.session.config().shell;
      if event != PromptEvent::Validate {
        return;
      }
      if input.is_empty() {
...
fn shell_impl(
  ...
  tokio::task::block_in_place(|| {
    helix_lsp::block_on(shell_impl_async(shell, cmd, input))
  })
}
...
fn shell(cx: &mut compositor::Context, cmd: &str, behavior: &ShellBehavior) {
  ...
  for range in selection.ranges() {
    ...
    if !success {
      cx.session.set_error("Command failed");
      return;
    }
...
fn add_newline_impl(cx: &mut Context, open: Open) {
  ...
  let changes = selection.into_iter().map(|range| {
    ...
    let pos = text.line_to_char(line);
    (pos, pos, Some(doc.line_ending.as_str().repeat(count).into()))
  });
...
fn increment_impl(cx: &mut Context, increment_direction: IncrementDirection) {
  ...
  for range in selection {
    ...
    match incremented {
      ...
      Some(new_text) => {
        ...
        cumulative_length_diff +=
          new_text.len() as i128 - selected_text.len() as i128;
        new_selection_ranges.push(new_range);
  ...
  if !changes.is_empty() {
    let new_selection =
      Selection::new(new_selection_ranges, selection.primary_index());
    let transaction = Transaction::change(doc.text(), changes.into_iter());
...
fn record_macro(cx: &mut Context) {
  if let Some((reg, mut keys)) = cx.session.macro_recording.take() {
    ...
    let s = keys
      ...
      .collect::<String>();
    match cx.session.registers.write(reg, vec![s]) {
      Ok(_) => cx.session.set_status(format!("Recorded to register [{}]", reg)),
      Err(err) => cx.session.set_error(err.to_string()),
    }
  } else {
    let reg = cx.register.take().unwrap_or('@');
    cx.session.macro_recording = Some((reg, Vec::new()));
    cx.session.set_status(format!("Recording to register [{}]", reg));
  }
}
...
fn replay_macro(cx: &mut Context) {
  ...
  if cx.session.macro_replaying.contains(&reg) {
    cx.session.set_error(format!(
    ...
    .map(|mut values| values.next().unwrap())
  {
    match helix_view::input::parse_macro(&keys) {
      Ok(keys) => keys,
      Err(err) => {
        cx.session.set_error(format!("Invalid macro: {}", err));
        return;
      },
    }
  } else {
    cx.session.set_error(format!("Register [{}] empty", reg));
    return;
  };

  // Once the macro has been fully validated, it's marked as being under replay
  // to ensure we don't fall into infinite recursion.
  cx.session.macro_replaying.push(reg);
  ...
  cx.callback.push(Box::new(move |compositor, cx| {
    ...
    cx.session.macro_replaying.pop();
  }));
}
