...
pub type SessionCompositorCallback =
  Box<dyn FnOnce(&mut Session, &mut Compositor) + Send>;
pub type SessionCallback = Box<dyn FnOnce(&mut Session) + Send>;
...
pub async fn dispatch(
  job: impl FnOnce(&mut Session, &mut Compositor) + Send + 'static,
) {
  let _ =
    JOB_QUEUE.wait().send(Callback::SessionCompositor(Box::new(job))).await;
}
...
pub fn dispatch_blocking(
  ...
  send_blocking(jobs, Callback::SessionCompositor(Box::new(job)))
}

pub enum Callback {
  SessionCompositor(SessionCompositorCallback),
...
impl Job {
  pub fn new<F: Future<Output = anyhow::Result<()>> + Send + 'static>(
    f: F,
  ) -> Self {
    Self { future: f.map(|r| r.map(|()| None)).boxed(), wait: false }
  }
  ...
  >(
    ...
    Self { future: f.map(|r| r.map(Some)).boxed(), wait: false }
  }
  ...
  pub fn handle_callback(
    ...
    match call {
      ...
      Ok(Some(call)) => match call {
        Callback::SessionCompositor(call) => call(editor, compositor),
        Callback::Session(call) => call(editor),
      },
      Err(e) => {
      ...
      match job {
        Ok(callback) => {
          ...
          if let Some(callback) = callback {
            ...
            match callback {
              Callback::SessionCompositor(call) if compositor.is_some() => {
