...
pub enum SnippetElement<'a> {
  Tabstop { tabstop: usize },
  Placeholder { tabstop: usize, value: Vec<SnippetElement<'a>> },
  Choice { tabstop: usize, choices: Vec<Tendril> },
  Variable { name: &'a str, default: Option<Vec<SnippetElement<'a>>>, regex: Option<Regex> },
  Text(Tendril),
}
...
fn render_elements(
  ...
  for element in snippet_elements {
    match element {
      ...
      Variable { name: _, regex: _, r#default } => {
      ...
      Placeholder { tabstop, value: inner_snippet_elements } => {
        let start_offset = *offset;
        if include_placeholder {
          render_elements(inner_snippet_elements, insert, offset, tabstops, newline_with_offset, include_placeholder);
        }
        tabstops.push((*tabstop, (start_offset, *offset)));
      },
      &Choice { tabstop, choices: _ } => {
        // TODO: choices
        tabstops.push((tabstop, (*offset, *offset)));
      },
    }
...
pub fn render(
  ...
  render_elements(&snippet.elements, &mut insert, &mut offset, &mut tabstops, newline_with_offset, include_placeholder);
  ...
  */
  ...
  fn var<'a>() -> impl Parser<'a, Output = &'a str> {
    ...
    move |input: &'a str| {
      input
        .char_indices()
        .take_while(|(p, c)| *c == '_' || if *p == 0 { c.is_ascii_alphabetic() } else { c.is_ascii_alphanumeric() })
        .last()
        .map(|(index, c)| {
  ...
  fn text<'a>(escape_chars: &'static [char], term_chars: &'static [char]) -> impl Parser<'a, Output = Tendril> {
    move |input: &'a str| {
  ...
  fn case_change<'a>() -> impl Parser<'a, Output = CaseChange> {
    ...
    choice!(map("upcase", |_| Upcase), map("downcase", |_| Downcase), map("capitalize", |_| Capitalize),)
  }
  ...
  fn format<'a>() -> impl Parser<'a, Output = FormatItem> {
    ...
    choice!(
      ...
      // '${' int ':' '/upcase' | '/downcase' | '/capitalize' '}'
      map(seq!("${", digit(), ":/", case_change(), "}"), |seq| { CaseChange(seq.1, seq.3) }),
      // '${' int ':+' if '}'
      map(seq!("${", digit(), ":+", text(TEXT_ESCAPE_CHARS, &['}']), "}"), |seq| {
        Conditional(seq.1, Some(seq.3), None)
      }),
      // '${' int ':?' if ':' else '}'
      map(
        seq!("${", digit(), ":?", text(TEXT_ESCAPE_CHARS, &[':']), ":", text(TEXT_ESCAPE_CHARS, &['}']), "}"),
        |seq| { Conditional(seq.1, Some(seq.3), Some(seq.5)) }
      ),
      // '${' int ':-' else '}' | '${' int ':' else '}'
      map(seq!("${", digit(), ":", optional("-"), text(TEXT_ESCAPE_CHARS, &['}']), "}"), |seq| {
        Conditional(seq.1, None, Some(seq.4))
      }),
    )
  }
  ...
  fn regex<'a>() -> impl Parser<'a, Output = Regex> {
    map(
      ...
      |(_, value, _, replacement, _, options)| Regex { value, replacement, options },
    )
  }

  fn tabstop<'a>() -> impl Parser<'a, Output = SnippetElement<'a>> {
    map(or(right("$", digit()), map(seq!("${", digit(), "}"), |values| values.1)), |digit| SnippetElement::Tabstop {
      tabstop: digit,
    })
  }
  ...
  fn placeholder<'a>() -> impl Parser<'a, Output = SnippetElement<'a>> {
    map(
      ...
      |seq| SnippetElement::Placeholder { tabstop: seq.1, value: seq.3 },
    )
  }

  fn choice<'a>() -> impl Parser<'a, Output = SnippetElement<'a>> {
    map(seq!("${", digit(), "|", sep(text(CHOICE_TEXT_ESCAPE_CHARS, &['|', ',']), ","), "|}",), |seq| {
      SnippetElement::Choice { tabstop: seq.1, choices: seq.3 }
    })
  }
  ...
  fn variable<'a>() -> impl Parser<'a, Output = SnippetElement<'a>> {
    choice!(
      // $var
      map(right("$", var()), |name| SnippetElement::Variable { name, default: None, regex: None }),
      // ${var}
      map(seq!("${", var(), "}",), |values| SnippetElement::Variable { name: values.1, default: None, regex: None }),
      // ${var:default}
      map(seq!("${", var(), ":", zero_or_more(anything(TEXT_ESCAPE_CHARS, true)), "}",), |values| {
        SnippetElement::Variable { name: values.1, default: Some(values.3), regex: None }
      }),
      // ${var/value/format/options}
      map(seq!("${", var(), regex(), "}"), |values| {
        SnippetElement::Variable { name: values.1, default: None, regex: Some(values.2) }
      }),
    )
  }

  fn anything<'a>(escape_chars: &'static [char], end_at_brace: bool) -> impl Parser<'a, Output = SnippetElement<'a>> {
  ...
  fn snippet<'a>() -> impl Parser<'a, Output = Snippet<'a>> {
    map(one_or_more(anything(TEXT_ESCAPE_CHARS, false)), |parts| Snippet { elements: parts })
  }

  pub fn parse(s: &str) -> Result<Snippet, &str> {
    snippet().parse(s).and_then(|(remainder, snippet)| if remainder.is_empty() { Ok(snippet) } else { Err(remainder) })
  }
  ...
  mod test {
    ...
    fn parse_placeholders_in_function_call() {
      assert_eq!(
        Ok(Snippet {
          elements: vec![
            Text("match(".into()),
            Placeholder { tabstop: 1, value: vec!(Text("Arg1".into())) },
            Text(")".into())
          ]
        }),
        parse("match(${1:Arg1})")
    ...
    fn unterminated_placeholder() {
      assert_eq!(
        Ok(Snippet { elements: vec![Text("match(".into()), Text("$".into()), Text("{1:)".into())] }),
        parse("match(${1:)")
      )
    }
    ...
    fn parse_empty_placeholder() {
      assert_eq!(
        Ok(Snippet {
          elements: vec![Text("match(".into()), Placeholder { tabstop: 1, value: vec![] }, Text(")".into())]
        }),
        parse("match(${1:})")
      )
    }
    ...
    fn parse_placeholders_in_statement() {
      assert_eq!(
        Ok(Snippet {
          elements: vec![
            Text("local ".into()),
            Placeholder { tabstop: 1, value: vec!(Text("var".into())) },
            Text(" = ".into()),
            Placeholder { tabstop: 1, value: vec!(Text("value".into())) },
          ]
        }),
    ...
    fn parse_tabstop_nested_in_placeholder() {
      assert_eq!(
        Ok(Snippet {
          elements: vec![Placeholder { tabstop: 1, value: vec!(Text("var, ".into()), Tabstop { tabstop: 2 },) },]
        }),
        parse("${1:var, $2}")
      )
    }
      ...
      assert_eq!(
        Ok(Snippet {
          elements: vec![Placeholder {
            tabstop: 1,
            value: vec!(Text("foo ".into()), Placeholder { tabstop: 2, value: vec!(Text("bar".into())) },),
          },]
        }),
        parse("${1:foo ${2:bar}}")
      )
      ...
      assert_eq!(
        Ok(Snippet {
          elements: vec![
            ...
            Tabstop { tabstop: 2 },
            Text(" ".into()),
            Choice { tabstop: 1, choices: vec!["one".into(), "two".into(), "three".into()] },
            Text(" ".into()),
            Variable { name: "name", default: Some(vec![Text("foo".into())]), regex: None },
            Text(" ".into()),
            Variable { name: "var", default: None, regex: None },
            Text(" ".into()),
            Variable { name: "TM", default: None, regex: None },
          ]
        }),
    ...
    fn robust_parsing() {
      ...
      assert_text("far{{id:bern {{id:basel}}}}boo", "far{{id:bern {{id:basel}}}}boo");
      assert_text("far{{id:bern {{id2:basel}}}}boo", "far{{id:bern {{id2:basel}}}}boo");
      assert_text("${}$\\a\\$\\}\\\\", "${}$\\a$}\\");
      assert_text("farboo", "farboo");
      assert_text("far{{}}boo", "far{{}}boo");
      assert_text("far{{123}}boo", "far{{123}}boo");
    ...
    fn parse_variable() {
      use SnippetElement::*;
      assert_snippet("$far-boo", &[Variable { name: "far", default: None, regex: None }, Text("-boo".into())]);
      assert_snippet("far$farboo", &[Text("far".into()), Variable { name: "farboo", regex: None, default: None }]);
      assert_snippet("far${farboo}", &[Text("far".into()), Variable { name: "farboo", regex: None, default: None }]);
      assert_snippet("$123", &[Tabstop { tabstop: 123 }]);
      assert_snippet("$farboo", &[Variable { name: "farboo", regex: None, default: None }]);
      assert_snippet("$far12boo", &[Variable { name: "far12boo", regex: None, default: None }]);
      assert_snippet(
        "000_${far}_000",
        &[Text("000_".into()), Variable { name: "far", regex: None, default: None }, Text("_000".into())],
      );
    }
    ...
    fn parse_variable_transform() {
      assert_snippet(
        ...
        &[Variable {
          name: "foo",
          regex: Some(Regex { value: Tendril::new(), replacement: Vec::new(), options: Tendril::new() }),
          default: None,
        }],
      );
      assert_snippet(
      ...
      assert_snippet(
        ...
        &[Variable {
          name: "foo",
          regex: Some(Regex { value: "m/atch".into(), replacement: vec![FormatItem::Capture(1)], options: "i".into() }),
          default: None,
        }],
      );
      ...
      assert_snippet(
        ...
        &[Variable {
          name: "foo",
          regex: Some(Regex { value: ".*".into(), replacement: vec![FormatItem::Capture(1)], options: "i".into() }),
          default: None,
        }],
      );
      assert_snippet(
      ...
      assert_snippet(
        ...
        &[Variable {
          ...
          regex: Some(Regex {
            value: ".*".into(),
            replacement: vec![FormatItem::Text("complex".into()), FormatItem::Conditional(1, Some("if".into()), None)],
            options: "i".into(),
          }),
          default: None,
        }],
      ...
      assert_snippet(
        ...
        &[Variable {
          ...
          regex: Some(Regex {
            value: ".*".into(),
            replacement: vec![FormatItem::Text("complex".into()), FormatItem::CaseChange(1, CaseChange::Upcase)],
            options: "i".into(),
          }),
          default: None,
        }],
      ...
      assert_snippet(
        ...
        &[Variable {
          ...
          regex: Some(Regex {
            value: "a".into(),
            replacement: vec![FormatItem::Text("in/".into()), FormatItem::Capture(1), FormatItem::Text("ner".into())],
            options: "g".into(),
          }),
          default: None,
        }],
