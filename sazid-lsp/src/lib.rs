...
use helix_core::syntax::{LanguageConfiguration, LanguageServerConfiguration, LanguageServerFeatures};
use helix_stdx::path;
...
pub mod util {
  ...
  pub fn diagnostic_to_lsp_diagnostic(
    ...
    let tags = if !new_tags.is_empty() { Some(new_tags) } else { None };
  ...
  pub fn lsp_pos_to_pos(doc: &Rope, pos: lsp::Position, offset_encoding: OffsetEncoding) -> Option<usize> {
    ...
    let pos = line.start.checked_add(pos.character as usize).unwrap_or(line.end).min(line.end);
  ...
  pub fn pos_to_lsp_pos(doc: &Rope, pos: usize, offset_encoding: OffsetEncoding) -> lsp::Position {
    match offset_encoding {
      OffsetEncoding::Utf8 => {
  ...
  pub fn range_to_lsp_range(doc: &Rope, range: Range, offset_encoding: OffsetEncoding) -> lsp::Range {
  ...
  pub fn lsp_range_to_range(doc: &Rope, mut range: lsp::Range, offset_encoding: OffsetEncoding) -> Option<Range> {
  ...
  fn find_completion_range(text: RopeSlice, replace_mode: bool, cursor: usize) -> (usize, usize) {
    let start = cursor - text.chars_at(cursor).reversed().take_while(|ch| chars::char_is_word(*ch)).count();
    let mut end = cursor;
    if replace_mode {
      end += text.chars_at(cursor).skip(1).take_while(|ch| chars::char_is_word(*ch)).count();
    }
    (start, end)
  }
  fn completion_range(
  ...
  pub fn generate_transaction_from_completion_edit(
    ...
    let replacement: Option<Tendril> = if new_text.is_empty() { None } else { Some(new_text.into()) };

    let text = doc.slice(..);
    let (removed_start, removed_end) =
      completion_range(text, edit_offset, replace_mode, selection.primary().cursor(text))
  ...
  pub fn generate_transaction_from_snippet(
    ...
    let (transaction, mut selection) = Transaction::change_by_selection_ignore_overlapping(
      ...
      |replacement_start, replacement_end| {
        ...
        let indent_level =
          helix_core::indent::indent_level_for_line(mapped_doc.line(line_idx), tab_width, indent_width) * indent_width;

        let newline_with_offset = format!("{line_ending}{blank:indent_level$}", line_ending = line_ending, blank = "");

        let (replacement, tabstops) = snippet::render(&snippet, &newline_with_offset, include_placeholder);
        ...
        off += replacement_start as i128 - replacement_end as i128 + replacement.len() as i128;
    ...
    for (range, (tabstop_anchor, tabstops)) in selection.into_iter().zip(selection_tabstops) {
      ...
      if range.head < range.anchor {
        ...
        if range.anchor > last_tabstop {
          ...
          mapped_selection.push(range);
          let rem_tabstops = tabstops[..last_idx].iter().map(|tabstop| Range::point(tabstop_anchor + tabstop.0));
        ...
        if range.anchor < first_tabstop {
          ...
          mapped_selection.push(range);
          let rem_tabstops = tabstops[1..].iter().map(|tabstop| Range::point(tabstop_anchor + tabstop.0));
          mapped_selection.extend(rem_tabstops);
          continue;
        }
      };

      let tabstops = tabstops.iter().map(|tabstop| Range::point(tabstop_anchor + tabstop.0));
  ...
  pub fn generate_transaction_from_edits(
    ...
    Transaction::change(
      ...
      edits.into_iter().map(|edit| {
        ...
        let replacement = if !edit.new_text.is_empty() { Some(edit.new_text.into()) } else { None };

        let start = if let Some(start) = lsp_pos_to_pos(doc, edit.range.start, offset_encoding) {
          start
        } else {
          return (0, 0, None);
        };
...
impl Registry {
  ...
  pub fn get_by_id(&self, id: usize) -> Option<&Client> {
    self.inner.values().flatten().find(|client| client.id() == id).map(|client| &**client)
  }
  ...
  fn start_client(
    ...
    let id = self.counter;
    self.counter += 1;
    let NewClient(client, incoming) = start_client(id, name, ls_config, config, doc_path, root_dirs, enable_snippets)?;
    ...
    language_config
      ...
      .filter_map(|LanguageServerFeatures { name, .. }| {
        if self.inner.contains_key(name) {
          let client = match self.start_client(name.clone(), language_config, doc_path, root_dirs, enable_snippets) {
            Ok(client) => client,
            error => return Some(error),
          };
          let old_clients = self.inner.insert(name.clone(), vec![client.clone()]).unwrap();
  ...
  pub fn get<'a>(
    ...
    language_config.language_servers.iter().map(move |LanguageServerFeatures { name, .. }| {
      if let Some(clients) = self.inner.get(name) {
        if let Some((_, client)) = clients
          .iter()
          .enumerate()
          .find(|(i, client)| client.try_add_doc(&language_config.roots, root_dirs, doc_path, *i == 0))
        {
          return (name.to_owned(), Ok(client.clone()));
        }
      }
      match self.start_client(name.clone(), language_config, doc_path, root_dirs, enable_snippets) {
        Ok(client) => {
          self.inner.entry(name.to_owned()).or_default().push(client.clone());
...
impl LspProgressMap {
  ...
  pub fn is_created(&mut self, id: usize, token: &lsp::ProgressToken) -> bool {
    self.0.get(&id).map(|values| values.get(token).is_some()).unwrap_or_default()
  }

  pub fn create(&mut self, id: usize, token: lsp::ProgressToken) {
    self.0.entry(id).or_default().insert(token, ProgressStatus::Created);
  }

  /// Ends the progress by removing the `token` from server with `id`, if removed returns the value.
  pub fn end_progress(&mut self, id: usize, token: &lsp::ProgressToken) -> Option<ProgressStatus> {
  ...
  pub fn update(
    ...
    self.0.entry(id).or_default().insert(token, ProgressStatus::Started(status))
  }
}
...
fn start_client(
  ...
  tokio::spawn(async move {
    ...
    let value = _client
      .capabilities
      .get_or_try_init(|| _client.initialize(enable_snippets).map_ok(|response| response.capabilities))
      .await;
    ...
    let notification_result = _client.notify::<lsp::notification::Initialized>(lsp::InitializedParams {}).await;

    if let Err(e) = notification_result {
      log::error!("failed to notify language server of its initialization: {}", e);
      return;
    }
...
pub fn find_lsp_workspace(
  ...
  for ancestor in file.ancestors() {
    if root_markers.iter().any(|marker| ancestor.join(marker).exists()) {
      top_marker = Some(ancestor);
    }

    if root_dirs.iter().any(|root_dir| path::normalize(workspace.join(root_dir)) == ancestor) {
    ...
    if ancestor == workspace {
      ...
      // is
      return top_marker.or_else(|| (!workspace_is_cwd).then_some(workspace)).map(Path::to_owned);
    }
  }
...
mod tests {
  ...
  fn emoji_format_gh_4791() {
    ...
    let edits = vec![
      TextEdit {
        range: Range { start: Position { line: 0, character: 1 }, end: Position { line: 1, character: 0 } },
        new_text: "\n  ".to_string(),
      },
      TextEdit {
        range: Range { start: Position { line: 1, character: 7 }, end: Position { line: 2, character: 0 } },
        new_text: "\n  ".to_string(),
      },
    ];
