use crate::resource_operations::Delete;
use crate::{
...
use lsp::{
  notification::DidChangeWorkspaceFolders, CodeActionCapabilityResolveSupport, DidChangeWorkspaceFoldersParams, OneOf,
...
impl Client {
  pub fn try_add_doc(
    ...
    let root = find_lsp_workspace(
      doc_path.and_then(|x| x.parent().and_then(|x| x.to_str())).unwrap_or("."),
      root_markers,
      manual_roots,
      &workspace,
      workspace_is_cwd,
    );
    let root_uri = root.as_ref().and_then(|root| lsp::Url::from_file_path(root).ok());

    if self.root_path == root.unwrap_or(workspace)
      || root_uri
        .as_ref()
        .map_or(false, |root_uri| self.workspace_folders.lock().iter().any(|workspace| &workspace.uri == root_uri))
    {
    ...
    let Some(capabilities) = self.capabilities.get() else {
      ...
      tokio::spawn(async move {
        ...
        {
          client.add_workspace_folder(root_uri, &workspace_folders_caps.change_notifications);
        }
      });
      return true;
    };
  ...
  fn add_workspace_folder(&self, root_uri: Option<lsp::Url>, change_notifications: &Option<OneOf<bool, String>>) {
  ...
  pub fn start(
    ...
    let (server_rx, server_tx, initialize_notify) = Transport::start(reader, writer, stderr, id, name.clone());
    let (workspace, workspace_is_cwd) = find_workspace();
    let workspace = path::normalize(workspace);
    let root = find_lsp_workspace(
      doc_path.and_then(|x| x.parent().and_then(|x| x.to_str())).unwrap_or("."),
    ...
    let workspace_folders = root_uri.clone().map(|root| vec![workspace_for_uri(root)]).unwrap_or_default();
  ...
  pub fn capabilities(&self) -> &lsp::ServerCapabilities {
    self.capabilities.get().expect("language server not yet initialized!")
  }

  pub(crate) fn file_operations_intests(&self) -> &FileOperationsInterest {
    self.file_operation_interest.get_or_init(|| FileOperationsInterest::new(self.capabilities()))
  }
  ...
  pub fn supports_feature(&self, feature: LanguageServerFeature) -> bool {
    ...
    match feature {
      LanguageServerFeature::Format => {
        matches!(capabilities.document_formatting_provider, Some(OneOf::Left(true) | OneOf::Right(_)))
      },
      ...
      LanguageServerFeature::GotoDefinition => {
        matches!(capabilities.definition_provider, Some(OneOf::Left(true) | OneOf::Right(_)))
      },
      LanguageServerFeature::GotoTypeDefinition => matches!(
        capabilities.type_definition_provider,
        Some(TypeDefinitionProviderCapability::Simple(true) | TypeDefinitionProviderCapability::Options(_),)
      ),
      LanguageServerFeature::GotoReference => {
        matches!(capabilities.references_provider, Some(OneOf::Left(true) | OneOf::Right(_)))
      },
      LanguageServerFeature::GotoImplementation => matches!(
        capabilities.implementation_provider,
        Some(ImplementationProviderCapability::Simple(true) | ImplementationProviderCapability::Options(_),)
      ),
      ...
      LanguageServerFeature::DocumentHighlight => {
        matches!(capabilities.document_highlight_provider, Some(OneOf::Left(true) | OneOf::Right(_)))
      },
      ...
      LanguageServerFeature::DocumentSymbols => {
        matches!(capabilities.document_symbol_provider, Some(OneOf::Left(true) | OneOf::Right(_)))
      },
      LanguageServerFeature::WorkspaceSymbols => {
        matches!(capabilities.workspace_symbol_provider, Some(OneOf::Left(true) | OneOf::Right(_)))
      },
      LanguageServerFeature::Diagnostics => true, // there's no extra server capability
      LanguageServerFeature::RenameSymbol => {
        matches!(capabilities.rename_provider, Some(OneOf::Left(true)) | Some(OneOf::Right(_)))
      },
  ...
  pub async fn workspace_folders(&self) -> parking_lot::MutexGuard<'_, Vec<lsp::WorkspaceFolder>> {
  ...
  /// Execute a RPC request on the language server.
  fn call<R: lsp::request::Request>(&self, params: R::Params) -> impl Future<Output = Result<Value>>
  where
    R::Params: serde::Serialize,
  {
    ...
    R::Params: serde::Serialize,
  {
    ...
    async move {
      ...
      server_tx.send(Payload::Request { chan: tx, value: request }).map_err(|e| Error::Other(e.into()))?;
  ...
  pub fn notify<R: lsp::notification::Notification>(&self, params: R::Params) -> impl Future<Output = Result<()>>
  where
    R::Params: serde::Serialize,
  {
    let server_tx = self.server_tx.clone();
    ...
    async move {
      ...
      server_tx.send(Payload::Notification(notification)).map_err(|e| Error::Other(e.into()))?;
  ...
  pub fn reply(
    ...
    async move {
      let output = match result {
        Ok(result) => {
          Output::Success(Success { jsonrpc: Some(Version::V2), id, result: serde_json::to_value(result)? })
        },
        Err(error) => Output::Failure(Failure { jsonrpc: Some(Version::V2), id, error }),
      };

      server_tx.send(Payload::Response(output)).map_err(|e| Error::Other(e.into()))?;
    ...
    let params = lsp::InitializeParams {
      ...
      capabilities: lsp::ClientCapabilities {
        workspace: Some(lsp::WorkspaceClientCapabilities {
          ...
          execute_command: Some(lsp::DynamicRegistrationClientCapabilities { dynamic_registration: Some(false) }),
          inlay_hint: Some(lsp::InlayHintWorkspaceClientCapabilities { refresh_support: Some(false) }),
        ...
        text_document: Some(lsp::TextDocumentClientCapabilities {
          completion: Some(lsp::CompletionClientCapabilities {
            completion_item: Some(lsp::CompletionItemCapability {
              ...
              tag_support: Some(lsp::TagSupport { value_set: vec![lsp::CompletionItemTag::DEPRECATED] }),
              ..Default::default()
            }),
          ...
          signature_help: Some(lsp::SignatureHelpClientCapabilities {
            signature_information: Some(lsp::SignatureInformationSettings {
              ...
              parameter_information: Some(lsp::ParameterInformationSettings { label_offset_support: Some(true) }),
  ...
  pub fn did_change_configuration(&self, settings: Value) -> impl Future<Output = Result<()>> {
    self.notify::<lsp::notification::DidChangeConfiguration>(lsp::DidChangeConfigurationParams { settings })
  }
  ...
  pub fn will_rename(
    ...
    let url_from_path = |path| {
      let url = if is_dir { Url::from_directory_path(path) } else { Url::from_file_path(path) };
      Some(url.ok()?.to_string())
    };
    let files = vec![lsp::FileRename { old_uri: url_from_path(old_path)?, new_uri: url_from_path(new_path)? }];
    let request = self.call_with_timeout::<lsp::request::WillRenameFiles>(lsp::RenameFilesParams { files }, 5);
  ...
  pub fn text_document_did_open(
    ...
    self.notify::<lsp::notification::DidOpenTextDocument>(lsp::DidOpenTextDocumentParams {
      text_document: lsp::TextDocumentItem { uri, language_id, version, text: String::from(doc) },
    })
  }
  ...
  pub fn text_document_did_change(
    ...
    Some(self.notify::<lsp::notification::DidChangeTextDocument>(lsp::DidChangeTextDocumentParams {
      text_document,
  ...
  pub fn text_document_did_close(
    ...
    self.notify::<lsp::notification::DidCloseTextDocument>(lsp::DidCloseTextDocumentParams { text_document })
  }
  ...
  pub fn text_document_did_save(
    ...
    let include_text = match &capabilities.text_document_sync.as_ref()? {
      lsp::TextDocumentSyncCapability::Options(lsp::TextDocumentSyncOptions { save: options, .. }) => {
        match options.as_ref()? {
          lsp::TextDocumentSyncSaveOptions::Supported(true) => false,
          lsp::TextDocumentSyncSaveOptions::SaveOptions(lsp_types::SaveOptions { include_text }) => {
    ...
    Some(self.notify::<lsp::notification::DidSaveTextDocument>(lsp::DidSaveTextDocumentParams {
      text_document,
  ...
  pub fn resolve_completion_item(
    ...
    match capabilities.completion_provider {
      Some(lsp::CompletionOptions { resolve_provider: Some(true), .. }) => (),
      _ => return None,
    }
  ...
  pub fn resolve_code_action(&self, code_action: lsp::CodeAction) -> Option<impl Future<Output = Result<Value>>> {
    let capabilities = self.capabilities.get().unwrap();
  ...
  pub fn text_document_signature_help(
    ...
    let params = lsp::SignatureHelpParams {
      text_document_position_params: lsp::TextDocumentPositionParams { text_document, position },
      work_done_progress_params: lsp::WorkDoneProgressParams { work_done_token },
      context: None,
  ...
  pub fn text_document_range_inlay_hints(
    ...
    match capabilities.inlay_hint_provider {
      Some(lsp::OneOf::Left(true) | lsp::OneOf::Right(lsp::InlayHintServerCapabilities::Options(_))) => (),
      _ => return None,
    }
  ...
  pub fn text_document_formatting(
    ...
    let options = if let Some(mut properties) = config_format {
      ...
      lsp::FormattingOptions { properties, ..options }
    } else {
      options
    };
  ...
  fn goto_request<
    T: lsp::request::Request<Params = lsp::GotoDefinitionParams, Result = Option<lsp::GotoDefinitionResponse>>,
  >(
    &self,
    text_document: lsp::TextDocumentIdentifier,
    position: lsp::Position,
  ...
  pub fn goto_definition(
    ...
    Some(self.goto_request::<lsp::request::GotoDefinition>(text_document, position, work_done_token))
  }
  ...
  pub fn goto_declaration(
    ...
    Some(self.goto_request::<lsp::request::GotoDeclaration>(text_document, position, work_done_token))
  }
  ...
  pub fn goto_type_definition(
    ...
    match capabilities.type_definition_provider {
      Some(lsp::TypeDefinitionProviderCapability::Simple(true) | lsp::TypeDefinitionProviderCapability::Options(_)) => {
        ()
      },
      _ => return None,
    }

    Some(self.goto_request::<lsp::request::GotoTypeDefinition>(text_document, position, work_done_token))
  }
  ...
  pub fn goto_implementation(
    ...
    match capabilities.implementation_provider {
      Some(lsp::ImplementationProviderCapability::Simple(true) | lsp::ImplementationProviderCapability::Options(_)) => {
        ()
      },
      _ => return None,
    }

    Some(self.goto_request::<lsp::request::GotoImplementation>(text_document, position, work_done_token))
  }
  ...
  pub fn prepare_rename(
    ...
    match capabilities.rename_provider {
      Some(lsp::OneOf::Right(lsp::RenameOptions { prepare_provider: Some(true), .. })) => (),
      _ => return None,
    }

    let params = lsp::TextDocumentPositionParams { text_document, position };
  ...
  pub fn code_actions(
    ...
    match capabilities.code_action_provider {
      Some(lsp::CodeActionProviderCapability::Simple(true) | lsp::CodeActionProviderCapability::Options(_)) => (),
      _ => return None,
    }
  ...
  pub fn did_change_watched_files(
    ...
    self.notify::<lsp::notification::DidChangeWatchedFiles>(lsp::DidChangeWatchedFilesParams { changes })
  }
}
