...
impl TreeData {
  pub fn new(parser: &mut Parser, path: PathBuf, language: Language) -> Result<Self, anyhow::Error> {
    ...
    match first_error {
      ...
      Some(_) => {
        ...
        if let Some(node) = first_error {
          ...
          output.push_str(&format!(" [{}, {}] - [{}, {}])", start.row, start.column, end.row, end.column));
        }
        output.push('\n');
        output
      },
