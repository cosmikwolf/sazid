...
impl FunctionCall for PatchFilesFunction {
  ...
  fn call(
    ...
    let patch_content: Option<&str> = function_args.get("patch_content").and_then(|s| s.as_str());
    // ensure the patches directory and session_data dir exist
    if let patches_dir = session_config.session_dir.join("patches") {
    ...
    match function_args.get("patch_name").and_then(|s| s.as_str()) {
      Some(patch_name) => {
        ...
        if let Some(patch_content) = function_args.get("patch_content").and_then(|s| s.as_str()) {
          create_patch_file_results = create_patch_file(patch_path, patch_content);
        }
      ...
      None => Ok(Some(format!("patch_name argument is required"))),
    }
  }
